...
plan for, see [§4](#the-development-workflow-with-ai)).
...
> replace) each repo's `AGENTS.md`, which carries the low-level
...
- **High quality bar, especially for core numerics.** Aim *higher* than
  ...
  reproducible and validated (see [§7](#gold-standard-validation)).
...
3. **Have CC implement the issue and open the PR.** Fill every section of the
   [PR template](#pull-requests) — including the cross-repo dependency table.
...
5. **Produce a NONMEM comparison** of the fit (see
   [§7](#gold-standard-validation)). Wire it into the test suite at the right tier:
...
- A **release is a tag on `main`** (e.g. `v0.3.0`); see [§9](#releases).
...
[§8](#cicd-and-quality-gates) and by treating tagged releases — not arbitrary
...
- **Cross-repo dependency table** — record the matching ferx-r (or ferx-core)
  ...
  required but not yet open, mark the PR **Draft**. See [§10](#cross-repo-synchronization).
- **Numerical validation** — reference values vs NONMEM / nlmixr2 / a prior
  ferx commit (see [§7](#gold-standard-validation)).
...
Tests live in three tiers (see `AGENTS.md` for placement rules). Put a new
...
See [§10](#cross-repo-synchronization).
...
.githooks`) and by the `fmt` CI job.

What the `Clippy` job actually fails on is narrower than "clippy is clean", and
worth knowing before you rely on it. `cargo clippy` exits 0 on warn-level
findings, so only two things redden the job: clippy's deny-by-default
`correctness` group, and rustc's `unused` group, which `tools/preflight.sh`
denies explicitly (`-Dunused`, #1470). That covers the leftovers a refactor drops — an
orphaned `use`, a field nothing reads, a variable nothing consumes — and, because
`-D` on a *group* also promotes its allow-by-default members, a `macro_rules!` arm
nothing invokes. It does not cover a `pub` item nobody calls, since `dead_code` never
fires on one, nor anything compiled out of the `ci,markov,nn` feature set. The rest of
clippy's warn-level surface (style, complexity, pedantic-adjacent perf lints) is
advisory: it is printed, it does not fail, and a green `Clippy` job is therefore
not a warning-free tree.
...
- On the R side, the `roxygen2`-generated `man/*.Rd` files **are** the docs —
  regenerate and commit them (see [§6](#pull-requests)).
- **`cargo test -p docs-lint` gates the structure** — section size, heading
...
Render locally with `tools/render-docs.sh` before marking documentation-heavy PRs
ready for review. Prefer it to a bare `quarto render docs`: Quarto's project
input discovery skips any path with a **hidden** (dot-prefixed) directory
component, so in a worktree under `.claude/worktrees/<name>/` — which
`AGENTS.md` asks for on any branch that is not `main` — `quarto render docs`
finds *zero* inputs. It writes `robots.txt` and `sitemap.xml`, renders not one
page, prints no warning and **exits 0**. The script stages `docs/` outside the
dot directory in that case, and fails loudly if a render produces no pages.
...
### 8.7 The public-API gate

The repo is a cargo workspace: the root package is the `ferx-core` **library**,
with `crates/ferx-tools` (multi-fit tooling — bootstrap, covariate search,
cross-validation) and `crates/ferx-cli` (the `ferx` binary) stacked on top. The
boundary rule is *`ferx-core` = one model, one dataset, one fit; `ferx-tools` =
many fits*. Statistical kernels stay in core; orchestration of repeated fits does
not.

Members can only reach `ferx-core` through its `pub` API, so the way that
boundary erodes is core growing an ad-hoc `pub` in the same PR that needs it.
`api/ferx-core-public-api.txt` is a committed `cargo public-api` snapshot and the
`Public API baseline` job diffs it, which turns every widening into one explicit
reviewable line rather than something buried in a large change. It doubles as
semver protection for the crates.io release and for ferx-r.

```bash
tools/update-public-api.sh          # regenerate after an intended change
tools/update-public-api.sh --check  # what CI runs
```

`#[doc(hidden)]` is banned outright (`tests/public_api_boundary.rs`): `cargo
public-api` omits such items, so the attribute is a working bypass of this gate,
not a style question. And anything `ferx-tools` needs must also be reachable from
ferx-r, which consumes `ferx-core` as an ordinary external crate — an API that
only makes sense "because we're in the same workspace" is the wrong API.
...
1. Confirm `main` is green (CI), ferx-core ↔ ferx-r are in sync, docs are
   ...
   *(Current: ferx-core `0.4.0`, ferx-r `0.4.0`.)*
...
change — `AGENTS.md` carries the rule. ferx-r keeps the equivalent `NEWS.md`, so
...
- [ ] Regenerate the public-API baseline in the same PR
      (`tools/update-public-api.sh`) and state which caller needs each added
      item — the `Public API baseline` job fails until you do. See
      [§8.7](#the-public-api-gate).
...
- **Sensitivity-reachable code:** keep functions on the `Dual2` path (the `*_g<T:
  ...
  `f64::max`/`min` where it matters; see "Analytic Sensitivities" in `AGENTS.md`.
...
- [ ] CI green: `check`, `test`, `survival`, `clippy`, `fmt`, `public-api`; coverage not degraded.
...
the fix** (see [§8](#cicd-and-quality-gates)). Correctness bugs in the core
...
trade-off ([§5](#git-workflow)). A cross-repo fix must keep ferx-core ↔ ferx-r
in sync ([§10](#cross-repo-synchronization)).
...
notes**, bump the version ([§9](#releases)), and be announced in the release
...
coordination via Google Chat ([§3](#communication)).
