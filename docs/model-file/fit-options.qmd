...
| Key | Values | Default | Description |
|-----|--------|---------|-------------|
| `ode_method` | `auto`, `rk45`, `vern7`, `rosenbrock23`, `rodas4`, `rodas5p` | `auto` | Which stepper integrates the `[odes]` block. Two axes: `vern7` (Verner 7(6), 10 stages) buys **order** for fits that are *accuracy*-limited at tight tolerances — measured 2.8× fewer steps and ~2.3× faster than `rk45` on the Savic transit NONMEM anchor at `1e-9`, but ~1.4× slower at default tolerances, so it is not a blanket upgrade. The Rosenbrock methods buy **stability**. `rk45` is the explicit Dormand-Prince method, the fastest choice on non-stiff models, and what the default `auto` selects on one. The other three are **linearly implicit Rosenbrock** methods for **stiff** systems — fast reversible binding / TMDD, Michaelis-Menten with `KM` far below observed concentrations, long transit chains, QSP cascades — where an explicit method is stability-limited and grinds down to the minimum step (visible as a fit that crawls, or as `ode_max_steps` being exhausted). Pick by tolerance: `rosenbrock23` (order 2, aliases `ros23`/`ode23s`) for crude tolerances or rough right-hand sides, `rodas4` (order 4) at typical `1e-6`–`1e-9`, `rodas5p` (order 5) at `ode_reltol ≤ 1e-9`. A stiff step costs `n + 1` extra right-hand-side evaluations for the finite-difference Jacobian plus one `n × n` factorization, so this is not a free upgrade — switch only when stiffness, not accuracy, caps the step. Read `n` as *the size of the system actually being integrated*, which is not always the `[odes]` state count: a **CTMM** endpoint integrates the occupancy system `dP/dt = P·Q(t)`, so an `s`-state chain is an `s²`-state integration and a Rosenbrock method there factors an `s² × s²` matrix per step attempt (a 5-state chain → a 25×25 LU). `ode_method` reaches that solve too, which is what you want when the chain has widely separated transition rates and is genuinely stiff, and is pure overhead otherwise. The stiff methods are full peers of `rk45` — every feature works with every method, including analytic sensitivities, non-Gaussian endpoints (TTE / categorical / CTMM), time-to-event simulation and adaptive dosing — because each method carries its own continuous extension for in-step state readouts. `auto` (the default) picks between the explicit and stiff families itself: before each integration it builds the Jacobian `J = ∂f/∂u` at that segment's own initial state and starts a stiff method (`rodas4`, or `rodas5p` at `ode_reltol ≤ 1e-8`) when the fastest mode `max |Re λ(J)|` is at least `30` per time unit, keeping `rk45` otherwise. Probing *per segment* rather than once per model is what makes it work on binding models, whose fast eigenvalue is carried by a term like `KON · central` and is therefore identically zero at the declared initial condition — the one state at which such a model looks non-stiff. An escalation that comes back non-finite, or that clamps at the minimum step, is discarded and re-solved explicitly, bounding the worst case to the explicit answer at twice the cost; that guard applies to `auto` only, so a named method is always honoured as asked. The threshold is a rate and so carries your model's time unit (calibrated for hours) — name a method explicitly on an unusual time scale. Measured on the cyclophosphamide model: converges in 0.7 s at OFV 3241.708 (NONMEM FOCEI: 3241.721) where `rk45` spends 827 s without finishing one optimizer iteration. See [ODE Models](ode-models.qmd#letting-ferx-pick-the-stepper-ode_method--auto). |
| `ode_auto_switch` | `true`, `false` | `true` | Let `ode_method = auto` change stepper **inside** an integration segment, not only at its start. The a-priori probe reads the Jacobian at the segment's entry state, which is the state a binding model looks least stiff at: both factors of a `KON · C · R` term are zero when a dose lands, so a depot-absorption binding model can read `|Re λ|max = 10` on entry and `7.6e3` an hour later. With this on, the probe is re-read every 25 accepted steps and the stepper is swapped in place — the segment keeps the state it has integrated so far and carries on with the other method, rather than restarting or re-solving. On that model the switch takes `auto` from a **143 % median relative error** (the explicit stepper exhausting `ode_max_steps`) to `7e-8`, in 90 steps instead of 10 000. Nothing else sees this failure: it produces **zero** minimum-step clamps and a *lower* step-rejection rate than the same model integrated accurately, so neither `ode_stiff_abort_after` nor any step-history counter can trigger on it. A switch onto a stiff method is guarded exactly like a start-of-segment escalation — if the stiff half comes back non-finite or clamps, the segment is discarded and re-solved explicitly (clamps taken by the *explicit* stepper before the switch do not count: they are the limit of the method the switch replaced). Like every stiffness decision `auto` takes, the switch is a **threshold** test, so it makes the objective *discontinuous* in θ wherever a segment sits on the boundary — a subject whose `|Re λ|max` hovers near the threshold at a re-probe can switch at θ and not at θ+h, and the two trajectories differ by far more than solver tolerance. The start-of-segment probe and the escalation guard already do this; re-probing adds up to eight more decision points per segment. Unlike `ode_stiff_abort_after` it is on by default, because the alternative on such a segment is not a slightly different answer but a silently wrong one (143 %, above). If a covariance step returns implausible standard errors on a model whose `ode_solver` warning reports mid-segment switches, re-run with `ode_auto_switch = false` — or with a named stiff method, which is pinned and takes no such decisions — to see whether the switching is what moved them. Ignored under a named `ode_method`, which is always pinned. Set `false` to go back to one method per segment, chosen at its start. |
...
| Key | Default | Description |
|-----|---------|-------------|
| `vi_iters` | `25000` | Adam **ceiling**, not a fixed budget: the run stops early once the objective and the estimates have both stopped moving, so a high ceiling costs an easy fit nothing while leaving head-room for a hard one. Settling is judged on a moving average of the ELBO against the trace's own noise, never on a per-iteration change — the objective is a Monte-Carlo estimate and such a test would stop on noise. `vi$n_iterations` reports what actually ran and `vi$converged` whether it settled; inspect `vi$elbo_trace` when it did not. |
| `vi_mc_samples` | `32` | Monte-Carlo draws per subject per iteration. This sets the noise floor the settling test measures against, so it decides *where the fit stops* rather than merely making it noisy — which is why the default is 32 and not the 8 it was: on `data/warfarin.csv` (~1% residual) 8 draws land σ 31% above the FOCEI/AGQ value with the OFV 9.6 units short, and the run is not reliably flagged (see the [VI page](../estimation/vi.qmd)). `32` closes that to +10% and 1.4, and `128` to +5% and 0.37. Cost is sublinear — a lower noise floor also settles sooner, so 4× the draws cost ~2.3× the wall time. Lower it only after checking σ against a `laplace`/`focei` fit of the same data. |
| `vi_lr` | `0.02` | Adam learning rate. Lower it if the trace oscillates, or if σ settles above a FOCEI/AGQ fit of the same data. |
| `vi_grad_clip` | `1e4` | Global-L2 gradient clip applied to every Adam step; `0` disables it. If the gradient vector is longer than this it is rescaled to this length, keeping its direction. The point is not to shorten the step — Adam normalises step size anyway — but to bound Adam's second moment, which averages *squared* gradients with a ~1000-iteration memory. A single enormous gradient (routine under proportional error when a prediction approaches zero) otherwise inflates it so far that every later step is numerically zero, freezing the fit; see [Variational inference](../estimation/vi.qmd). The value is not a sensitive knob — anything from `1` to `1e5` behaves the same — because clipping is asymptotically a no-op. |
