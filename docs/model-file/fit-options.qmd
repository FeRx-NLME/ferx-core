...
SIR is skipped entirely — with a warning saying so — in only two cases: the
covariance step was never run (`covariance = false`), or it ran and the
Hessian's eigendecomposition did not produce finite eigenvalues (NaN/Inf
entries), leaving no proposal to draw from.
