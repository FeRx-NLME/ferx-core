...
| `analytic_cov_hessian` | `true`, `false` | `true` | Assemble the covariance R-matrix (Hessian) in **closed form** — the exact analytic FOCE/FOCEI Hessian (Almquist `M2 + M3` for FOCEI, the Sheiner–Beal marginal Hessian for FOCE, both including the EBE-response curvature) — instead of by finite differences. Being finite-difference-free, it has no `fd_hessian_step` to tune and no catastrophic cancellation on flat directions, and it runs ~20× faster (FOCEI; ~9× FOCE) than the OFV-Hessian stencil while reproducing NONMEM `$COV MATRIX=R` to 5–6 significant figures at the same point. Used automatically for in-scope analytical-PK models; out-of-scope subjects (ODE, scaling, LTBS, IOV, residual-error random effects, BLOQ, time-varying covariates, oral-infusion, resets) transparently fall back to the finite-difference covariance. When on, it supersedes `covariance_ofv_hessian` and `fd_hessian_step` for in-scope models. Set `false` to force the finite-difference path. Requires `covariance = true`. |
