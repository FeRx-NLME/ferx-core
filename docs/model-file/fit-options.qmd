...
These six keys (and the two stepper keys below) can also arrive from a caller — `ferx_fit(settings = ...)` in R, or a `FitOptions` field passed to the Rust `fit()`. Precedence is **per key**: a key the caller sets wins over the model file for that fit, and a key the caller leaves at its default yields to the model file, so passing a hand-built settings object cannot silently loosen a model that pinned `ode_reltol = 1e-10`. The consequence of that rule: asking explicitly for a default value (`ode_reltol = 1e-4`) reads the same as not asking, and the model file still wins. Put it in `[fit_options]` when a model needs a specific accuracy regardless of who runs it.
