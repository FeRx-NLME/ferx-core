...
| `vi_mc_samples` | `8` | Monte-Carlo draws per subject per iteration. This sets the noise floor the settling test measures against, so too few draws stop the fit *early* rather than merely making it noisy: on warfarin at `8`, σ lands 34% above the FOCEI/AGQ value and the OFV 11.3 units short. `32` recovers the reference. When a run stops with drift still visible but `elbo_tightness_ratio` is near 1, ferx reports `converged: false` — raise draws first, or lower `vi_lr`. A ratio above 25 instead signals a bad basin; improve the population starts or use FOCEI/SAEM rather than adding draws. |
