...
| `outer_fd_method` | `fixed`, `shi`, `gill` | `fixed` | Scalar outer-FD interval policy. `fixed` preserves the historical centred stencil; `shi` is bounded central search; experimental `gill` is forward. Adaptive modes require `outer_fd_noise_abs`. |
| `outer_fd_noise_abs` | positive float | unset | Absolute scalar-objective noise bound required by adaptive outer FD. |
...
| `inner_fd_method` | `fixed`, `shi` | `fixed` | Inner FD interval policy. `shi` adapts scalar posterior gradients and prediction Jacobians; it requires both noise settings. |
| `inner_fd_objective_noise_abs` | positive float | unset | Absolute scalar-posterior noise bound for adaptive inner gradients. |
| `inner_fd_prediction_noise_abs` | positive float | unset | Absolute prediction noise bound for adaptive inner Jacobians. |
