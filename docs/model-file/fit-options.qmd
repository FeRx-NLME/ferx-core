...
| `impmap_rb_recenter_blend` | `1.0` | FREM Rao-Blackwell proposal recenter stabilization (issue #678), **experimental, opt-in**. `impmap` re-solves the full joint (PK + covariate) MAP mode every iteration; on the Rao-Blackwell path (`frem_rao_blackwell`) that fresh estimate can drift/wobble between comparable local optima or noisy curvature estimates iteration to iteration, which the ISCALE pilot search (`iscale_min`/`iscale_max`) can't correct since it only rescales proposal width, not center/shape. A value below `1.0` convex-blends the fresh per-iteration joint-MAP estimate with the previous iteration's own joint-MAP estimate to damp that drift; `0.0` centers purely on the previous iteration's estimate. Must be in `(0, 1]`; the default `1.0` is a no-op that reproduces `impmap`'s original behaviour exactly. Unlike other stabilization options in this table, this one has **not** been validated to actually help on a real high-dimensional FREM fit — treat it as an experimental knob to try, not a proven fix. |
