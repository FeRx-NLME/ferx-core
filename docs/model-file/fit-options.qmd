...
| `vi_mc_samples` | `8` | Monte-Carlo draws per subject per iteration. This sets the noise floor the settling test measures against, so too few draws change *where the fit stops* rather than merely making it noisy: on `data/warfarin.csv` (~1% residual) at `8` draws, σ lands 31% above the FOCEI/AGQ value with the OFV 9.6 units short; `32` closes that to 1.4 and `128` to 0.37. When a run stops with drift still visible in the trace ferx reports `converged: false` and says so in `warnings`, but it can also settle at a biased fixed point where nothing flags it — so on a residual error under ~3%, raise this (or check σ against a `laplace`/`focei` fit) rather than trusting `converged`. See the [VI page](../estimation/vi.qmd). |
