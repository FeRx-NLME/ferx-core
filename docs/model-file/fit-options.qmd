...
| `mstep_damping` | `0.03` | Exploration-phase cap on the stochastic-approximation step for the numerical θ/σ M-step, the θ-side counterpart of `omega_burnin`'s protection for Ω. The M-step result is blended in as `θ ← θ + γ_θ·(θ* − θ)` rather than assigned, because assigning it outright is `argmax` of a *single* MCMC η draw and biases a theta that carries no ETA. Must be in `(0, 1]`; smaller damps harder, `1.0` disables the damping entirely. Ignored (with a warning) when every theta is mu-referenced or `FIX`, and for mixture models. See the [SAEM page](../estimation/saem.qmd). |
