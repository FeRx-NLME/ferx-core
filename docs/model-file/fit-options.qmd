...
| `outer_fd_method` | `fixed`, `shi`, `gill` | `fixed` | Interval policy for the scalar packed-space finite-difference fallback used when an outer analytic gradient is unavailable. `fixed` preserves the historical centred `1e-4 * (1 + |x|)` stencil. `shi` is a bounded noise-aware centred interval search; `gill` is experimental and uses a separate forward-difference curvature/noise interval. Either adaptive choice requires `outer_fd_noise_abs`; it does not infer objective noise from an ODE tolerance or inner-loop tolerance. These controls affect only the outer scalar fallback, not covariance, prediction-Jacobian, or analytic-gradient paths. |
| `outer_fd_noise_abs` | positive float | unset | Absolute error bound in the evaluated scalar objective's units for `outer_fd_method = shi` or `gill`. Required for either adaptive policy. Estimate this deliberately for the objective being differenced; an ODE relative tolerance is not an OFV error bound. |
...
| `inner_fd_method` | `fixed`, `shi` | `fixed` | Interval policy for numerical inner derivatives when the analytic provider cannot serve a subject. `shi` uses a bounded centred search for both the scalar posterior gradient and prediction Jacobian. It requires both inner noise settings below. |
| `inner_fd_objective_noise_abs` | positive float | unset | Absolute noise bound in the scalar individual posterior objective's units for adaptive inner gradients. |
| `inner_fd_prediction_noise_abs` | positive float | unset | Absolute noise bound in prediction units for adaptive inner Jacobians. This is intentionally separate from objective noise. |
