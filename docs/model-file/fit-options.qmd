...
- `auto` (the default) chooses between `nlopt_lbfgs` and `bobyqa` from the model:
  ...
  the gradient-based `nlopt_lbfgs`; otherwise — out-of-scope ODE/PD models, SDE,
  or `gradient = fd`, where the outer loop runs on finite differences — it falls
  back to the derivative-free `bobyqa`. A `[mixture]` model always gets `bobyqa`
  under `auto`. Benchmarking across ~10 real FOCEI datasets
