...
| `scale_adaptation` | `interval` | How the MH step scales are adapted. `interval` is the historical rule (×1.1 / ×0.9 every `adapt_interval`); `robbins_monro` steps `log δ` every iteration by `c·k^-0.6·(accept − target)`. The interval rule can only move `δ` by `1.1^n` / `0.9^n` over the `n` times it fires, which on a 400-iteration run is 8 — not enough reach for a model whose optimal step is an order of magnitude from the start. See the [SAEM page](../estimation/saem.qmd#adaptive-step-sizes). |
| `scale_deadband` | `none` | Acceptance window `lo,hi` in which the `robbins_monro` step is **skipped**, leaving the scale untouched; outside it the ordinary step applies. `none` (the default) steps on every iteration. Ignored under `scale_adaptation = interval`. A band wider than the spread of the per-iteration rate freezes the scales rather than disabling the band, so `0,1` is rejected — see the [SAEM page](../estimation/saem.qmd#scale_deadband-only-correct-a-chain-that-is-off-target). |
