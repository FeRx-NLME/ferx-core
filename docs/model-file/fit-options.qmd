...
| `outer_fd_method` | `fixed`, `shi`, `gill` | `fixed` | Interval policy for **numerical** outer gradients. `fixed` is the historical stencil, unchanged. Experimental — see [Noise-aware FD intervals](#noise-aware-finite-difference-intervals). |
| `outer_fd_noise_abs` | positive float | unset | Absolute OFV noise bound. Required (positive, finite) by `outer_fd_method = shi`/`gill`. |
...
| `inner_fd_method` | `fixed`, `shi` | `fixed` | Interval policy for **numerical** inner derivatives — the EBE posterior gradient and the prediction Jacobian, on models outside the analytic sensitivity scope. `fixed` (default) is unchanged. `shi` requires both `inner_fd_*_noise_abs` keys. Experimental — see [Noise-aware finite-difference intervals](#noise-aware-finite-difference-intervals). |
| `inner_fd_objective_noise_abs` | positive float | unset | Absolute noise bound on one subject's posterior objective. Required by `inner_fd_method = shi`; must be positive and finite. |
| `inner_fd_prediction_noise_abs` | positive float | unset | Absolute noise bound on a single prediction, in DV units. Required by `inner_fd_method = shi`; must be positive and finite. |
...
| `mstep_damping` | `0.03` | Exploration-phase cap on the stochastic-approximation step for the numerical θ/σ M-step, the θ-side counterpart of the per-iteration SA step cap SAEM already applies to Ω. The M-step result is blended in as `θ ← θ + γ_θ·(θ* − θ)` rather than assigned, because assigning it outright is `argmax` of a *single* MCMC η draw and biases a theta that carries no ETA. Sigma comes out of the same joint NLopt solve and takes the same step. Must be in `(0, 1]`; smaller damps harder. `1.0` is a sentinel rather than a cap value: it disables the damping entirely — in the convergence phase as well as exploration, restoring the pre-damping outright assignment throughout — so the option is discontinuous there (`0.999` still gives the full convergence schedule). Ignored (with a warning naming the reason) when every estimated theta is mu-referenced or `FIX`, when `mu_referencing = false`, and for mixture models. See the [SAEM page](../estimation/saem.qmd). |
...
| Key | Default | Description |
|-----|---------|-------------|
| `vi_iters` | `25000` | Adam **ceiling**, not a fixed budget — the run stops early once the objective and the estimates have both settled. `vi$n_iterations` reports what actually ran and `vi$converged` whether it settled; inspect `vi$elbo_trace` when it did not. See [Why there is no convergence tolerance](../estimation/vi.qmd#why-there-is-no-convergence-tolerance). |
...
## Noise-aware finite-difference intervals {#noise-aware-finite-difference-intervals}

*Experimental (#1314).* Every key here defaults to `fixed`, which is the
historical behaviour, bit-for-bit.

ferx computes most gradients analytically (`sens/`), but some derivatives are
necessarily numerical: the outer gradient of models outside the analytic scope,
the reconverged-EBE outer gradient, and the inner EBE gradient / prediction
Jacobian when the analytic provider does not cover a model. Those use a
prescribed interval — `1e-4·(1 + |x|)` outer, `1e-7·(1 + |x|)` inner. A
prescribed interval is a bet that the objective is smooth at that scale. On a
genuinely noisy objective (ODE solver tolerance, an FD-inner FOCE marginal) the
bet can be wrong in either direction: too small and the difference is
noise, too large and it is a chord across curvature.

`shi` instead *searches* for the interval, using the Shi–Xie–Xuan–Nocedal test:
widen or narrow until the stencil's third difference is comparable to — but not
swamped by — the noise you declare. `gill` is a bounded one-sided variant that
balances curvature against noise. Both need you to state the noise: there is no
way to infer it from a single evaluation.

### Choosing the noise bounds

The bound is **absolute**, in the units of the thing being differenced, and it
is a bound on the *error of one evaluation*, not a standard deviation.

- `outer_fd_noise_abs` is in OFV units. For an ODE model, the practical floor is
  set by `ode_reltol`/`ode_abstol`: an OFV of order 1000 solved at
  `ode_reltol = 1e-6` is not reproducible below roughly `1e-3`. Start there and
  compare fits.
- `inner_fd_objective_noise_abs` is one subject's posterior objective, so it is
  smaller than the OFV bound by roughly the subject count.
- `inner_fd_prediction_noise_abs` is one prediction, in DV units.

ferx does **not** guess these. `outer_fd_method = shi` with no
`outer_fd_noise_abs` is rejected before the fit starts
(`E_OUTER_FD_NOISE_REQUIRED`), as is a zero or non-finite one — a zero bound
would make every candidate interval unusable and quietly disable the search.

### When it is worth turning on

Rarely, and the threshold is measurable. The fixed stencil's noise-driven error
is about `ε/h`, so against a gradient of size `|g|` it only matters once
`ε ≳ |g|·h` — with the outer `h ≈ 2e-4` that is an OFV noise of order `1e-3`.

Measured on `examples/mm_iv.ferx` (Michaelis-Menten ODE, FD inner gradients),
worst relative outer-gradient error against each objective's own true gradient,
with controlled noise of amplitude `ε` added:

| `ε` (OFV units) | `fixed` | `shi` | `gill` |
|---|---|---|---|
| 1e-6 | 8.6e-4 | 8.6e-4 | 4.7e-3 |
| 1e-4 | 1.1e-2 | 6.9e-5 | 1.2e-2 |
| 1e-2 | 1.1e0 | 2.4e-3 | 2.6e-1 |
| 1e0 | 1.1e2 | 1.2e-1 | 2.0e0 |

`fixed` degrades linearly in `ε`, as `ε/h` predicts; `shi` holds the gradient to
three digits where `fixed` has lost it entirely, and is ~1000× more accurate at
the top of that range.

That is the case *for* the option. The case for leaving it off is the same
model's real noise: measured at `3e-8`–`3e-6` across `ode_reltol` from 1e-12 to
1e-3 and `inner_tol` to 1e-2 — one to three orders of magnitude below the
threshold. In that regime `shi` matches `fixed` to within a factor of two (and
was slightly worse in two of five settings, from growing `h` into curvature),
so it buys nothing. **Measure your objective's noise before enabling this.** If
it is far below `|g|·h`, the fixed stencil is already resolving your gradient.

`gill` was worse than `fixed` at every noise level tested, including 300× worse
on a clean objective — it is one-sided, and its interval comes from an estimated
second derivative that is itself noise-limited. Prefer `shi`.

### What happens when no interval fits

A search can fail to place a stencil: the coordinate sits against a bound, every
trial point is rejected by the EBE guard, or the interval stops being
representable in floating point. In that case the coordinate falls back to the
**fixed** stencil for that gradient evaluation. It is never left at zero — a
zero derivative does not read as "unknown" to an optimizer, it reads as
"stationary", and a gradient of them is reported as convergence.

A near-quadratic coordinate is a legitimate non-acceptance too: its third
difference never rises above the noise however wide the interval grows, so the
search returns its widest bound-limited interval rather than one the test
endorsed.

### Scope

`inner_fd_method` is honoured wherever a fit re-solves EBEs from its
`[fit_options]` — the FOCE/FOCEI objective and gradient, the reconverged-EBE
gradient, SAEM/IMP/VI/SIR inner loops, the covariance step and post-hoc EBEs.
Utility paths with no `FitOptions` of their own (`simulate()`, the starting-value
sweep) always use the fixed policy.

Neither key affects an analytic gradient. If a model is fully inside the
analytic sensitivity scope, setting these changes nothing.
