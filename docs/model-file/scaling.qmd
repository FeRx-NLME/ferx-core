...
1 and `W_CMT_DEFAULTED` reports that separately; this code is what
catches a column that is present and simply never says `2`. Validating a
model without `--data` reports nothing here: an empty population is no
evidence about which entries are exercised.
