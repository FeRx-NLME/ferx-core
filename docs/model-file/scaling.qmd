...
::: {.callout-important}
## Bioavailability and lag are an error here, not a warning

The volume case above is a *warning* because a further division by `V` can be a
deliberate transform. A **dose attribute** is not: if the `pk(...)` call maps
`f=F` or `lagtime=TLAG`, the engine already applies that value at the dose, so an
`obs_scale`/`y` that also reads it applies it twice — measured at exactly `F` on
the prediction. There is no reading of that model under which the doubling is
intended, so it is rejected with `E_DOSE_ATTR_DOUBLE_USE`
([#1004](https://github.com/FeRx-NLME/ferx-core/issues/1004)) rather than warned
about. The fix is to drop the `f=`/`lagtime=` mapping if the parameter is an
ordinary one — renaming does not help, because the mapping follows the
parameter — or to drop the read if it really is the dose attribute.

NONMEM computes the same doubled value silently: `$PK` with both `F1` and
`S2 = V/F1` runs clean. That is the `CL/F`, `V/F` apparent-parameter convention
being written *on top of* a separately-defined `F1`, which double-counts there
too; ferx is deliberately stricter.
:::
...
::: {.callout-note}
## A model with no compartments belongs in `[structural_model]`, not here

Form C is a readout **on top of** a compartment model. If the prediction is an
equation with no compartments underneath — a dose-response curve, a response
time-course, a meta-regression — write it directly in `[structural_model]`
instead; it uses this same grammar, including the named intermediates and
continuation lines described below. See [Compartment-Free Models](structural-model.qmd#compartment-free-models-the-pred-equivalent).
A `[scaling]` block alongside such a model is rejected: that block already
declares the prediction.
:::
...
**The other direction is a warning** (since [#1405](https://github.com/FeRx-NLME/ferx-core/issues/1405)) — a
`[CMT=N]` entry that *no* observation is recorded on is inert rather than
wrong, so `fit()` and `ferx check --data` report `W_PER_CMT_UNMATCHED`
naming the dead compartments instead of refusing the model. It fires on
all three per-CMT channels, `obs_scale[CMT=N]`, `y[CMT=N]` and
`[error_model] CMT=N:`. The usual cause is a `CMT` column that never
carries the declared value — a typo, an endpoint removed by
`[data_selection]`, or a block carried over from another model. On a
dataset with *no* `CMT` column every observation is keyed to compartment
1 and **both** codes fire: `W_CMT_DEFAULTED` for the defaulting itself,
this one for the entries the defaulting left dead. The message offers
that reading only when the data is consistent with it — every
observation on compartment 1 — so a model whose observations sit on
compartment 2 is told its entries are inert without being sent to audit
a `CMT` column that is present and correct. When a `[data_selection]`
clause emptied one of the dead compartments the message says so and
names it; a clause that removed rows on some *other* compartment is not
offered as the cause. Validating a model without `--data` reports
nothing here: an empty population is no evidence about which entries are
exercised.
