...
The converse — one variable mapped to two **different** roles, `pk one_cpt_oral(cl=CL, v=V, ka=KA, f=X, lagtime=X)` — is also a parse error (issue #1359). Both slots are written, but the model's per-parameter slot table can hold only one role per variable, so which one it recorded depended on hash order and the lag-aware / lag-free code paths could be chosen differently between two runs of the same file. Give each role its own variable (`F = X` and `LAGTIME = X` in `[individual_parameters]`, then `f=F, lagtime=LAGTIME`).
