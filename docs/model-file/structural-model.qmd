...
Because nothing is dosed, the dose-attribute names a `pk(...)` or `[odes]` model
reserves — `F`, `LAGTIME`, and the compartment-indexed `F1`, `ALAG1`, `D2`, `R1`
— carry no meaning here and are ordinary parameter names (a fraction, a factor
level, a slope). They are neither bound to a dose route nor rejected for lacking
one (#1358).
