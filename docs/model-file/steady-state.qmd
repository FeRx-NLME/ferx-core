...
measured, an explicit `TAFD` train moves 29 % per doubling of its length while the `TAD`
train moves 3.5e-7. `TIME`/`T` under `SS=1` matches NONMEM (both engines equilibrate on a
cycle-local clock and then integrate forward on absolute time, so both sit the same
distance from their own non-convergent train), and `TAFD` under `SS=1` still reads `NaN`.
Neither number is the steady state of a system that has one; if your model means an
absolute clock, expand the run-in with explicit dose records instead of `SS=1`.
