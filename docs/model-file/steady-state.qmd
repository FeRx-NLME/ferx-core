...
the steady state of a system that has one. `TAFD` has no such convention in either engine:
the periodic fiction has no first dose, so there is no finite value to hand back and the
solve returns `NaN` — including when the term's coefficient is zero, since `0.0 * NaN` is
`NaN`. Giving it the run-in's own origin instead would silently redefine `TAFD` as `TAD`.

Since ferx 0.3 this combination is reported as `W_STEADY_STATE_ABSOLUTE_TIME` by both
`fit()` and `ferx check` (see
[Check report](../file-formats/check-report.qmd#codes-data-and-dosing)). It is a **warning**,
not a rejection — the model still runs, and for `TIME`/`T` it still matches NONMEM. If your
model means an absolute clock, expand the run-in with explicit dose records instead of
`SS=1`; if it means a per-dose clock, use `TAD`, which does have a periodic steady state.
