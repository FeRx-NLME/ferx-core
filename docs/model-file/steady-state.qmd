...
The truncation tail after `N` cycles is bounded by `exp(-N·λ_slow·II)`, where
`λ_slow` is the **slowest** disposition rate constant. For typical PK
(`λ_slow·II ≈ 2`, i.e. ~3 half-lives per dosing interval), `exp(-100) ≈ 4e-44` —
well below any meaningful precision.

It is **not** always negligible, and the governing quantity is `λ_slow·II`, not the
number of compartments. A one-compartment oral model with `CL = 0.1`, `V = 50`
(t½ ≈ 350 h) dosed `II = 12` has `λ_slow·II ≈ 0.024`, giving `exp(-1.2) ≈ 0.30` — a
**30 %** difference from the exact periodic steady state. A two-compartment model
with a slow, deep peripheral (`Q = 0.5`, `V2 = 500`) reaches ~9 % at the same `II`.
Both are measured, not estimated. As a rule of thumb the truncation is negligible
once `λ_slow·II ≳ 0.5` and can reach tens of percent when it is not.

This matters only when a subject is served by the **pulse-train** path. The
dose-superposition closed form uses the exact periodic solution and is unaffected;
the event-driven walk and the ODE path use the pulse train. A subject is routed to
the walk by a time-varying covariate, an `EVID=3/4` reset, IOV, or (since #375) a
dose into a non-default compartment — so the same dataset can move between the two
representations. **No warning is emitted** when the tail is significant.
...
#519/#532, so the loop is no longer unconditionally fixed-length. The early stop
fires for fast PK; for the slow cases above it never trips and the full 50 cycles
run, which is exactly when the residual tail is largest.
...
a hot spot, a tighter early-stop (loop until `||state - prev|| / ||state||
