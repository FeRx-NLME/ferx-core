...
1. For each subject, random effects are sampled from $\eta_i \sim N(0, \Omega)$.
   ...
   `$SIM`. The post-fit NPDE/NPD diagnostics are a separate code path that
   draws its own per-occasion kappas the same way; see
   [fit options](fit-options.qmd#simulation-based-diagnostics-npde-npd).
