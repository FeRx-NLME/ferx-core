...
error[E_ETA_NOT_DECLARED] parameters: Model references ETA_CL as a random effect but
no `omega` / `kappa` declaration defines it. Declare the random effect in
[parameters] (e.g. `omega ETA_CL ~ 0.09`), or — if this is meant to be a data
column — add that column to the data file.
```
