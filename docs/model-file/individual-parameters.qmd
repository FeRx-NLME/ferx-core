...
correct `[odes]` block (RHS or `init(...)` seed), `[scaling]` readout, or
`[adaptive_dosing] observe` signal. Declaring one *and* referencing it on
...
`[initial_conditions]` is *not* one of those surfaces: an initial condition is not
an absorbed dose, so the engine deposits the amount with `F = 1` and no lag, and
`init(depot) = F * 500` applies `F` exactly once.
