...
- The weight may reference **covariates, thetas and `TIME`** — not random
  effects. A weight is the known precision of an occasion; a random effect there
  would make the variance of κ a function of κ.
- A weighted kappa may be referenced **only in `[individual_parameters]`**.
  Elsewhere it would read as the unweighted κ, so the parser rejects it.
...
- A weight that is zero, negative or non-finite at any record is a fit-time
  ...
  cell. A weight that changes *within* an occasion warns
  (`W_KAPPA_WEIGHT_VARIES_WITHIN_OCCASION`): κ is drawn once per occasion, so
  `Ω_IOV/W` is ambiguous when `W` moves underneath it.
