...
| `dose` | Each administration begins a new occasion. An observation takes the occasion of the most recent dose at or before its time (dose-before-observation at equal `TIME`). ADDL-expanded doses each start their own occasion. |
...
**Note.** For `time(...)`, observations are bucketed by the data-file `TIME` (the raw, user-clock time); doses use their event time. This is the common case; for reset-stacked subjects whose `TIME` restarts, prefer an explicit `iov_column`.
