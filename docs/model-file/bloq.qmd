...
| another whole number (`7`, `-2`, `200`) | scored by its **sign**: a positive flag like `1` (the lower tail), a negative one like `-1` (the upper tail) | `W_CENS_UNEXPECTED`, once per sign per subject |
| anything else (`1.5`, `abc`, `inf`) | **rejected** | an error naming the subject, the row's time and the cell |

The reject applies only to a cell the fit would actually consume: an observation row that survives `[data_selection]`. A row a clause removes, and a dose row (where ferx never reads `CENS`), are not checked — as in NONMEM, which rejects a text cell only on a record its `IGNORE` statements keep. A whole number outside `i8` saturates to ±127 and keeps its sign.
