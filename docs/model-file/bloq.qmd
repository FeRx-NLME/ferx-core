...
A cell that is **not a whole number**, such as `1.5`, `abc` or `inf`, is an error on a Gaussian observation row the fit scores. The message names the subject, the row's time and the cell. Correct the cell; if the column is not a censoring flag at all, rename it, in the dataset or aside in the model's [`[data]`](data.qmd) block. The same cell on a dose row, on a time-to-event or categorical row, or on a row that [`[data_selection]`](data-selection.qmd) removes is never read and is not an error. NONMEM 7.6.0 behaves the same way for `abc`: it rejects it on an observation record and accepts it on a record its `IGNORE` removes. NONMEM does accept `1.5`, as a number whose meaning your own code decides; ferx gives `CENS` a fixed meaning, so it rejects it, as it rejects `SS=1.5` ([#1496](https://github.com/FeRx-NLME/ferx-core/issues/1496)).
