...
falls back to the numerical M-step under `method = saem` / `imp`, which is
slower but correct — the covariate effect is in the expression either way. The
parser says so in a warning naming the parameter. FOCE/FOCEI are unaffected.
:::
...
range is fine (`linear(center = 0) +` is just the uncentred slope `θ·COV`). A
covariate that is **constant** in the data is still rejected when the centre is
symbolic, since `θ·(c₀ − c)` is then a constant shift confounded with the
