...
`1 + θ_k > 0`. An additive (`+`) relation has no factor: its θ is a shift in the
parameter's own units, and the block does not know that parameter's scale. So
the additive defaults drop the data-derived bounds entirely:
