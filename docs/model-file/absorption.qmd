...
On the mis-specified ridge ferx's derivative-free outer optimiser stalls only
slightly short of NONMEM's gradient FOCEI (≈ 1.5 OFV units, milder than `igd`'s
~18), so — as for `igd` — the implementation check is the **objective at the
