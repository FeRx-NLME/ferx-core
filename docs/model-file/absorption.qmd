...
In the meantime, the Weibull forcing is validated in-repo without NONMEM by: the
absorption-independent **mass-balance invariant** `∫A dt = F·Dose/ke`
(`tests/weibull_absorption.rs`), which catches a missing forcing, a
double-counted bolus, or a mis-normalised density end-to-end through the ODE
pipeline; and a unit-level **analytic-`Dual2` ≡ central-FD gradient** check on the
forcing (`src/pk/absorption.rs`) plus the full ODE-integrated parity in
`src/sens/ode_provider.rs`. The slow-gated `tests/weibull_nonmem_anchor.rs`
follows once the licensed NONMEM run lands (the same staging as transit, where the
feature shipped before its NONMEM anchor).
