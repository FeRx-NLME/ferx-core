...
>
> **ferx is stricter than NONMEM here.** NONMEM lets a `$PK` `F1 = …` be
> referenced in `$DES` and quietly computes `F²`. That divergence is the entire
> point of [#993](https://github.com/FeRx-NLME/ferx-core/issues/993) — but it
> means a mechanically translated control stream can newly fail to parse even
> though it ran in NONMEM. Anyone converting control streams (including
> `ferxtranslate`) should drop the `F` from the translated RHS rather than
> transcribe `$DES` literally.
