...
|---|---|---|---|---|
| `rk45` | 3 940 | 120 | 0 | 30.1 ms |
| **`vern7`** | **1 420** | 140 | 0 | **20.9 ms** |
| `rodas5p` | 4 060 | 180 | 0 | 79.7 ms |
| `rodas4` | 16 200 | 120 | 0 | 272.2 ms |
| `rosenbrock23` | 85 180 | 120 | 0 | 633.2 ms |

`vern7` takes 2.8× fewer steps and is ~1.4× faster: higher order is what an accuracy-limited
fit rewards. `rodas5p` takes the *same* number of steps as `rk45` and is 2.6× slower, because
each step now buys a Jacobian and a factorization the fit does not need — and the lower-order
stiff methods are worse still, ordered exactly as their convergence orders predict.

The crossover is real, so do not switch blindly: on the same model at **default** tolerances
`vern7` is ~1.7× *slower* than `rk45` (740 → 520 steps, but 10 stages per step instead of 6),
