...
Measured on the ferx-testdata ODE library (one prediction pass, default tolerances):

| model | `λ_max` range across segments | what `auto` starts | vs `rk45` |
|---|---|---|---|
| cyclophosphamide (`run2_final`) | 35 – 234 | `rodas4` | **6.2 ms → 1.0 ms**, predictions agree to 8 digits |
| cyclophosphamide (`run1_initial`) | 20 – 176 | `rodas4` | **4.4 ms → 1.0 ms** |
| TMDD SAD (`cr`, `crib`, `ib`) | 1.7 – 126 | mixed, per segment | within run-to-run noise; predictions agree to 7 digits |
| busulfan, pembrolizumab | 0.1 – 4.6 | `rk45` (no escalation) | unchanged |

The cyclophosphamide models are the clear win: they are stiff at every segment and an explicit
method is stability-limited throughout. The TMDD models are the interesting case — they are
stiff on the high-dose segments and not on the low-dose ones, which is exactly the situation a
single per-model choice cannot serve and a per-segment probe can.
