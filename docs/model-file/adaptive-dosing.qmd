...
each segment is integrated and fires only when that segment's state came back
non-finite *and* the clock the RHS reads was unanchored there. A `TAD` that appears
in a branch the pre-dose window does not take, or only inside a condition, is
therefore left alone — such a run integrates normally and is not refused. A RHS that
