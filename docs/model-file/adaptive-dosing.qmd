...
regimen to anchor them — `TAD` and `TAFD` have no referent at all. `predict()` fills
that window by looking ahead to the first dose in the whole record, which is a peek a
causal simulation has not earned: the dose in question has not been decided yet.

So `simulate_adaptive()` **refuses the window with a typed error** rather than
integrate an unanchored clock. The message names the segment, the spelling that is
...
the first dose has landed — would inherit it. A RHS that reads only `TIME` / `T` is
unaffected, since that clock is the integration axis and is anchored everywhere.
