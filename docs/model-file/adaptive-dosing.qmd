...
- **Pre-scheduled base regimens are supported** (#702) — the base subject may carry a
  loading / maintenance regimen, including a steady-state (`SS=1`) dose, which the driver
  integrates and the controller augments at the decision schedule (the real TDM / MIPD
  workflow: start on a fixed regimen, then titrate on measured levels). Base doses flow
  ...
  pre-scheduled + reactive bookkeeping is checked bit-for-bit each run. *Scope:* supported
  on constant-covariate, non-reset models — a base regimen combined with time-varying
  covariates, IOV, or system resets (EVID=3/4) is a typed error (each a #702 follow-up),
  never a silent mis-integration of a delivered dose.
