...
- **System resets (EVID=3) are supported** — the reactive driver zeros the
  ...
  row carries a dose, which the dose-free-base rule above rejects, so only pure
  EVID=3 resets reach the adaptive path.) *Caveat:* declaring `auc_target` on a reset
