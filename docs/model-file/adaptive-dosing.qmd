...
The check uses a tolerance tied to the ODE solver's own error control, not
bit-equality: a held decision makes the reactive driver restart the integrator at
a point the static schedule has no break for, which perturbs the adaptive step
sequence at the solver's error level. A real bookkeeping bug moves a prediction
by orders of magnitude more, so the check still catches it.
