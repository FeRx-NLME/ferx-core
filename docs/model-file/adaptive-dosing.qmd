...
and re-solving that same segment with the clock anchored comes back *finite*. The
third one is a counterfactual, and it is what makes the message's claim true rather
than plausible — a compartment that diverges for its own reasons, on a model that
merely mentions `TAD` somewhere, is not refused and not blamed on the clock. A `TAD`
that appears in a branch the pre-dose window does not take, or only inside a
condition, is likewise left alone. A RHS that reads only `TIME` / `T` is unaffected
in every case, since that clock is the integration axis and is anchored everywhere.
...
which way that comparison fell, and it will not match `predict()` on the realized
schedule. The **frozen-schedule replay verifier**, which is on by default, is what
catches this: it reports the divergence between the reactive walk and the replay,
without naming the clock as the cause. Turning it off (`verify: false`) on a model
that reads `TAD` or `TAFD` inside a comparison removes the only net there. Gating on
*evaluation* instead — a per-segment "this slot was loaded" flag inside the RHS
evaluator — would cover this shape too, and is tracked separately.
