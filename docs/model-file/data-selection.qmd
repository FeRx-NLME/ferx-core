...
record, so `ignore = CMT == 2` silently keeps a row it was meant to drop, and
`accept = CMT == 1` silently keeps one it was meant to exclude. Measured on a
one-compartment model with `ignore = CMT == 2` and a single observation cell
spelled `2` against `x`: 3 records scored and an objective of −4.6162 against 4
records and −6.9517.
...
already gets. Write the compartment explicitly in every row you intend to filter
on.
