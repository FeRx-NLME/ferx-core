...
column) is not read as anything. A rule that reads that column on that record —
whether it would remove the record or keep it — is an error naming the rule, since
it would otherwise decide the record on a value the dataset does not hold
([#1501](https://github.com/FeRx-NLME/ferx-core/issues/1501)). A rule on another
column may still remove the record, and then the cell is never read, as with
NONMEM's `IGNORE`; see [A cell that is not a number](../data-format.qmd#a-cell-that-is-not-a-number).
