...
| Name | Meaning |
|------|---------|
| Individual-parameter names (`CL`, `V`, …) | EBE-derived value for the subject — **every** name in `[individual_parameters]`, including an intermediate the `[structural_model]` line does not bind (`TVCL = THCL * 3`) and a modeled-dose `D{n}` / `R{n}`. Each is re-evaluated per row with that row's η, covariates and `TIME` |
...
- **SDE models** (`[diffusion]` block): `compartments[i]` returns the
  **deterministic ODE state** at each observation time. Today that is the only
  state there is — the EKF propagates the state covariance and never corrects
  the mean with the observed data, so there is no filtered trajectory for it to
  differ from (see
  [What the filter does not do](diffusion.qmd#what-the-filter-does-not-do)).
  The distinction becomes real when the mean update lands.
