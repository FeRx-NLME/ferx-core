...
- **`block_sigma` correlated residuals are supported** together with a
  ...
  unchanged.
- **Replicate samples at the same time** (two physical draws of a subject at one
  `TIME`, each written as a total + unbound row) are paired **one-to-one in row
  order** — the first total pairs with the first unbound, and so on — so the
  dense `R` stays block-diagonal in disjoint sample pairs and positive-definite
  (#827). ferx has no `L2` data item; instead it relies on the standard NONMEM
  layout where the two rows of one sample are written contiguously. Order the
  paired rows per sample (total then unbound, sample after sample) and the
  pairing matches NONMEM's `L2` grouping exactly. Before #827, all co-temporal
  rows were cross-correlated all-to-all, which made `R` indefinite and collapsed
  the objective.
