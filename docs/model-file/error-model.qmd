...
The fix is mechanical — reorder the `sigma` declarations, or reorder the arguments, so the two lists agree.
...
A sigma declared *after* the ones the error model names is untouched by this rule — it simply occupies a later slot that the error model never loads, and it still earns the usual "declared but not referenced in `[error_model]`" warning. This is the shape used by, for example, FREM models, where a trailing sigma is consumed by machinery other than `[error_model]`.

Per-CMT and covariate-selected error models (below) resolve each endpoint's sigmas by name, so declaration order does not constrain them.
