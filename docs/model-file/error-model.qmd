...
- Like any magnitude, it is supported for `method = foce` and `method = focei`
  only; SAEM, GN, GN-hybrid and importance sampling are rejected up front rather
  than silently ignoring the weight. The inner η-gradient and outer θ/σ gradient
  stay analytic.
