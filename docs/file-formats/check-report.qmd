...
search inside, before the first objective evaluation. All four codes below are
data-independent, so `ferx check model.ferx` reports them without a `--data`
file. All but `E_INIT_BOUNDS_INVERTED` are skipped when `maxiter = 0`: an
evaluation-only run clamps the start like any other, it just does not hold the
clamped value through a search.
