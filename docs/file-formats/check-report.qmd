...
| `E_DOSE_ATTR_NONFINITE` | error | A dose attribute the engine applies at the dose event — lag time (`lagtime` / `ALAG{n}`) or bioavailability (`F` / `F{n}`) — is `NaN` or infinite at typical values for some subject. The message names the subject, the compartment and which attribute. Usually a covariate relationship undefined for that subject; the common one is an exponential model on an unscaled covariate (`ALAG1 = TVLAG*exp(WT)`), since `exp` is the one DSL function with no domain guard — `/`, `ln` and `sqrt` are all floored. A non-finite lag makes `dose.time + lag`, and every break derived from it, non-finite, so the subject's whole integration timeline cannot be ordered and every prediction is `NaN`; this rejects it up front rather than after a fit of `NaN` objectives ([#1189](https://github.com/FeRx-NLME/ferx-core/issues/1189)). |
