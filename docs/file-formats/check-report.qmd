...
A `[parameters]` line must also be consumed end to end by exactly one declaration form. Trailing text after a complete declaration is reported as `E_PARSE` with the offending line quoted; the one shape with a code of its own is a scale tag on a `block_*` declaration, below. Before 0.4.0 both were dropped without a diagnostic.
...
| `E_BLOCK_VARIANCE_ONLY` | error | A scale tag (`(sd)`, `(variance)` or `(var)`) on a `block_omega`, `block_sigma` or `block_kappa` declaration. The lower triangle mixes variances and covariances, so one tag cannot say which entry is on which scale: square each SD into a variance and write the off-diagonals as covariances. Until 0.4.0 the tag was dropped in silence and the file reported VALID, so an SD-coded block fitted from squared-wrong initial estimates. See [#1377](https://github.com/FeRx-NLME/ferx-core/issues/1377) and [Parameters](../model-file/parameters.qmd). |
