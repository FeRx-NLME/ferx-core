...
| `E_SS_LAGTIME_TAD_RHS` | error | An `SS=1` dose with a **lagtime** on a model whose `[odes]` RHS reads `TAD`. `TAD` has no referent between the dose record and the lagged arrival, and the wrong value reaches the steady-state trough — shifting *every* prediction (measured 2.7 %), not only those inside that window. Structural: it asks whether a lagtime is *declared*, since an estimated `ALAG` moves during a fit. Raised by `fit()` and `ferx check` only — `simulate()`/`predict()` validate a narrower bundle and return `NaN` instead. Without a lagtime `TAD` is fine — see [Steady-state doses](../model-file/steady-state.qmd#limitations). |
