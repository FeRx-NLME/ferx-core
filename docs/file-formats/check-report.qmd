...
| `E_DOSE_ATTR_DOUBLE_USE` | error | A dose attribute the engine applies at the dose event is *also* read by the model, so its value is applied twice. `F`, `LAGTIME`/`ALAG`, `F{n}`/`ALAG{n}`/`LAGTIME{n}` are caught at parse time (message names `[odes]`, `[scaling]` or `[adaptive_dosing]`); `D{n}`/`R{n}` only when a coded `RATE=-2`/`-1` dose in the data lands on them. See [#993](https://github.com/FeRx-NLME/ferx-core/issues/993). |
