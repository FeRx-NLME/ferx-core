...
| `W_CENS_UNEXPECTED` | warning | An observation row's `CENS` cell carries a value other than `-1` (above ULOQ), `0` (quantified) or `1` (below LLOQ). `m3_logcdf` reads only the flag's sign — a positive flag is the lower tail, like `1`, and a negative one is the upper tail, like `-1` — so under `bloq_method = m3` such a row is scored as censored rather than rejected; the warning exists so that coercion is not silent. Reported once per subject, naming the subject and the first such value. |
