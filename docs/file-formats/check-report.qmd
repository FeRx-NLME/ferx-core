...
| `W_CENS_UNEXPECTED` | warning | An observation row's `CENS` cell carries a value other than `-1` (above ULOQ), `0` (quantified) or `1` (below LLOQ). The M3 likelihood's `m3_logcdf` treats **every** nonzero flag as a left tail, so such a row is scored as censored rather than rejected; the warning exists so that coercion is not silent. Reported once per subject, naming the subject and the offending value. |
