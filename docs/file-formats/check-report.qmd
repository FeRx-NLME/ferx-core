...
| `W_COMPARTMENT_FREE_DOSES` | warning | The dataset carries dose records but the `[structural_model]` is compartment-free (`$PRED`-equivalent), so no dose is applied and `AMT` / `RATE` / `SS` on those rows have no effect on the prediction. Reported once with the row and subject counts. A coded `RATE=-1` / `-2` row on such a model is covered here rather than by `E_MODELED_*_NO_PARAM`, which would ask for a `D{n}` / `R{n}` parameter the model cannot consume ([#1443](https://github.com/FeRx-NLME/ferx-core/issues/1443)). |
