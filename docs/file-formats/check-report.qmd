...
| `W_CENS_UNEXPECTED` | warning | An observation row's `CENS` cell carries a whole number other than `-1` (above ULOQ), `0` (quantified) or `1` (below LLOQ). `m3_logcdf` reads only the flag's sign — a positive flag is the lower tail, like `1`, and a negative one is the upper tail, like `-1` — so under `bloq_method = m3` such a row is scored as censored rather than rejected; the warning exists so that coercion is not silent, and names the tail the row is scored on. Reported once **per sign** per subject, naming the first such value of that sign, and quoting it **as written** — a value outside `i8` saturates to ±127 before it is scored, so `200` is reported as `CENS=200`, not `CENS=127`. A cell that is not a whole number at all (`1.5`, `abc`) is not this warning but an `E_DATA` **error**: it has no sign to be scored by. Both apply only to a row a fit will score; a dataset read for simulation is not checked, since nothing there reads the flag. |
