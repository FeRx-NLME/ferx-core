...
- **`laplace`, `n_agq = 1` (exact `H`)** — needs the full third-order jet, so each
  ...
  materially fewer calls. Whether that nets out was representation-dependent, and on
  a **diagonal** Ω the two costs used to be equal by construction, which is why
  successive benchmark sessions on that fixture measured opposite signs. The
  **block-Ω** case is where the call-count gap is real — 20 FD rebuilds against 13
  provider evaluations — and it measured −24% provider time on 5 of 5 reps at an
  identical outer-iteration count and identical OFV. That made it the default too.
...
So **both anchors now use the analytic route by default.** Both compute the same
derivative, so this is a cost choice, not an accuracy one: every benchmarked arm
returned an identical OFV. On an ODE `laplace` fit the route does change the
