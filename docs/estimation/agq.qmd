...
- **`focei`, `n_agq > 1` (Gauss-Newton `H̃`)** — `H̃ = Ω⁻¹ + Σⱼ pⱼaⱼaⱼᵀ` is bilinear
  ...
  ambiguity. This is the default for this anchor.
- **`laplace`, `n_agq = 1` (exact `H`)** — needs the full third-order jet, so each
  analytic evaluation costs materially more than a plain anchor rebuild; whether
  that is a net win is representation-dependent (diagonal vs block Ω, closed-form
  vs ODE) and close enough to parity to need re-measuring rather than trusting one
  past run. This route stays opt-in.

`FERX_AGQ_GRID_RESPONSE` overrides the per-anchor default: `fd` forces the
finite-difference sweep, `analytic` forces the analytic route, and any other value
(including an unset-but-misspelled one, e.g. `analytical`) is rejected outright
rather than silently falling back to the default — a benchmark's "candidate" arm
that quietly becomes identical to its baseline would otherwise report "no
difference" instead of an error.
