...
There is no separate `method = agq`: adaptive Gauss–Hermite quadrature is
`method = laplace` with `n_agq > 1` (the exact Hessian anchor). `n_agq = 1` (the
default) is the Laplace approximation.
...
`method = laplace` with the default `n_agq = 1` **is** the Laplace approximation:
one node, no grid, the exact-Hessian anchor. Raising `n_agq` turns the same
objective — same analytic gradient, same covariance step, same code — into adaptive
Gauss–Hermite quadrature. Prefer `n_agq = 1` when you want a
NONMEM-`LAPLACIAN`-comparable fit or a baseline to raise `n_agq` from.

It is the cheapest configuration — one node, no grid — and on warfarin it converges
*faster than FOCEI* (0.23 s vs 0.60 s), because the analytic gradient needs fewer
outer iterations.

The **Hessian anchor** is what the method name selects: `method = laplace` anchors
the grid on the exact conditional Hessian, `method = focei` on the Gauss-Newton
`H̃`. So `method = focei` with `n_agq > 1` is the Gauss-Newton-anchored quadrature —
FOCEI refined toward the exact marginal. Both anchors converge to the same integral
as `n_agq → ∞`; they differ only in where the nodes are placed (and, at one node, in
whether the `½log|H|` term carries the exact curvature or the Gauss-Newton
approximation).
...
5. **Outer loop.** AGQ has an **analytic outer gradient** (below), so
   ...
   works on the AGQ objective too, so the reported standard errors belong to
   the likelihood that was actually optimised — see
   [the analytic covariance](#the-analytic-covariance) for when that is computed
...
`∂nll(η_j)/∂θ` is available analytically from the `Dual2` sensitivity provider (the
...
The grid sweep itself is contracted once, not once per parameter: the node
gradients, weights and positions do not depend on which parameter is being
differentiated, so the two weighted moments `Σⱼ wⱼgⱼ` and `Σⱼ wⱼgⱼzⱼᵀ` are formed
once and each parameter then costs `O(d²)` — `O(Q·d² + n_free·d²)` rather than
`O(n_free·Q·d²)` for `Q = n_agq^d` nodes and `d` random effects. At `n_agq = 1`
every node sits at `z = 0`, so the node-displacement half is identically zero and
the matrix products behind it are skipped rather than computed and multiplied by
zero. Neither changes the number of node evaluations, which is what dominates on
an ODE model.

A fully analytic `dH/dθ` needs third derivatives (`∂³nll/∂η²∂θ`); the provider's
ordinary second-order call stops at `∂²f/∂η∂θ`, so by default that one order is
finite-differenced, as described above.

### Analytic instead of finite-difference `dH/dθ`

Both anchors also have an analytic route for that last order, replacing the
`2 × n_free` perturbed-anchor rebuilds with a fixed number of extra provider calls:

- **`focei`, `n_agq > 1` (Gauss-Newton `H̃`)** — `H̃ = Ω⁻¹ + Σⱼ pⱼaⱼaⱼᵀ` is bilinear
  in *first*-order sensitivities, so `dH̃/dθ` needs only the mixed second-order
  blocks (`∂²f/∂η∂θ`, `∂²f/∂η²`) the provider already returns for FOCEI's own
  `theta_block`/`sigma_block` terms — no third order at all. One extra ordinary
  sensitivity call replaces the whole FD sweep, at zero call-count-vs-per-call-cost
  ambiguity. This has always been the default for this anchor.
- **`laplace`, `n_agq = 1` (exact `H`)** — needs the full third-order jet, so each
  analytic evaluation costs materially more than a plain anchor rebuild, against
  materially fewer calls. Whether that nets out is representation-dependent, and on
  the measured **diagonal** Ω fixture the call counts are similar:
  `n_free = n_theta + d + n_sigma = 3 + 3 + 1 = 7` gives 14 FD rebuilds versus
  `1 + 2(n_theta + d) = 13` provider evaluations. These are different work units;
  similar counts imply neither equal runtime nor parity for diagonal Ω in general.
  Successive benchmark sessions on this fixture measured opposite timing signs. The **block-Ω**
  case is where the call-count gap is real — 20 FD
  rebuilds against 13 provider evaluations — and it measured −24% provider time on
  5 of 5 reps at an identical outer-iteration count and matching OFV to four decimals. That made it
  the default too.

  The sweep then dropped to **`1 + 2·n_eta` evaluations** (13 → 7 on a 3-θ/3-η
  model): it reads `∂³f/∂η³` and `∂³f/∂η²∂θ`, and the θ axes existed only to build
  blocks the covariance step consumes. `∂³f/∂η²∂θ` comes instead from `∂/∂η` of
  `∂²f/∂η∂θ` — the same mixed partial, a cheaper difference direction. That moved
  even the diagonal fixture decisively off parity: **−36%** provider time against
  the finite-difference route, where it had been −5%.

So **both anchors now use the analytic route by default.** The direct grid-response
parity test compares both anchors at `n_agq = 1` and `3` with a `1e-5` scaled-error
tolerance; separate Hessian-derivative tests cover block Ω, combined error, and ODE
models (the ODE test uses `5e-3`). These tests provide the derivative correctness
evidence. Benchmark OFVs agree at the console's four-decimal precision; that
agreement does not establish exact equality or gradient equivalence.

On an ODE `laplace` fit the route does change the
optimizer's path — the benchmark fixture converges in 37 outer iterations instead of
56 — so converged estimates can move within the convergence tolerance.

`FERX_AGQ_GRID_RESPONSE` is retained as a benchmarking escape hatch rather than
removed: `fd` forces the finite-difference sweep, `analytic` is now also the
default, and any other value (including an unset-but-misspelled one, e.g.
`analytical`) is rejected outright rather than silently falling back — a benchmark's
"candidate" arm that quietly becomes identical to its baseline would otherwise
report "no difference" instead of an error. It is deliberately an environment
variable and not a `[fit_options]` key: there is nothing here for a *user* to
choose.

The finite-difference sweep is also still the live fallback, not dead code: the
analytic route declines — out of its narrower scope (M3 censoring, say), or on an
ill-conditioned anchor — and the FD sweep then runs exactly as before.
...
## The analytic covariance

The analytic R-matrix supports three estimation paths:

| Fit | Covariance objective |
|---|---|
| `method = foce` | FOCE marginal with residual variance frozen at zero random effects |
| `method = focei`, `n_agq = 1` | FOCEI marginal |
| `method = focei`, `n_agq > 1` | FOCEI-anchored quadrature marginal, including movement of the nodes |

Outside the analytic scope, the covariance step uses finite differences of the
objective function. Under AGQ that is expensive: the stencil needs
`~2·n_free²` **reconverged population objectives**, and every one of them sweeps the
whole `n_agq^d` node grid for every subject. Doubling the nodes per dimension
multiplies the grid size by `2^d`.

For `method = focei` with `n_agq > 1`, ferx now assembles that R-matrix analytically
instead, in a single pass. The standard errors mean the same thing they did before —
they describe the quadrature marginal the fit actually minimised, not the FOCEI one.

### Why quadrature support uses the FOCEI anchor

The restriction is structural rather than a staging decision. The quadrature grid is
scaled by a Hessian anchor, and the two methods anchor differently:

| method | anchor | second derivative needs |
|---|---|---|
| `focei`, `n_agq > 1` | Gauss-Newton `H̃ = Ω⁻¹ + Σⱼ pⱼaⱼaⱼᵀ` | **third**-order sensitivities |
| `laplace`, any `n_agq` | exact `H = ∂²nll/∂b²` | **fourth**-order sensitivities |

`H̃` is built from first derivatives of the prediction, so differentiating it twice in
the population parameters bottoms out at third-order sensitivities — which ferx already
computes for the FOCE/FOCEI covariance. The exact conditional Hessian already contains
`∂²f/∂η²`, so the same operation would need a fourth order that does not exist. So
`method = laplace` keeps the finite-difference covariance at every node count.

### When it applies

The analytic route is taken when **all** of the following hold; otherwise the fit
keeps the finite-difference covariance, subject to its numerical convergence and step-size limits.

- `method = focei` with `n_agq > 1`.
- The model is inside the analytic covariance scope shared with FOCE/FOCEI, with a
  Gaussian endpoint the closed-form or ODE sensitivity provider reaches. M3/BLOQ censoring is supported;
  TTE, categorical endpoints, and custom or time-varying residual
  magnitudes are outside it.
- Closed-form and ODE IOV are supported, including correlated IOV blocks shared across
  occasions. The assembly uses the full joint mode and quadrature grid and ties
  the shared covariance derivatives across occasions. IOV with modeled lag,
  LTBS, or observation scaling remains outside this covariance scope.
- The subject's anchor is well conditioned. A flat or unidentifiable random-effect
  direction is declined rather than reported: the assembly would otherwise invert a
  near-singular matrix and return standard errors inflated by orders of magnitude.

That last point is worth stating plainly — **this does not make every AGQ fit cheaper**.
It speeds up the covariance step for Gaussian-endpoint FOCEI-anchored quadrature, which
is a common and expensive case, but the non-Gaussian endpoints that motivate quadrature
in the first place keep the existing path.

### Covariance validation

For IOV, `tests/iov_analytic_covariance.rs` compares FOCEI R-matrix standard errors
at the stored NONMEM 7.6.0 beta 1 `MATRIX=R` estimates for warfarin with two
occasions. All eight SEs agree within 0.44% (the test allows 1%); OFVs are
308.83426 in ferx and 308.83047 in NONMEM. The control stream and output are
stored as `tests/nonmem/warfarin_iov_cov_r.{ctl,ext}`. FOCE and FOCEI IOV Hessians
are also checked against reconverged analytic gradients, including correlated
BSV and IOV blocks; AGQ-FOCEI is checked against its reconverged quadrature
objective. The legacy objective stencil was numerically indefinite at this
reference point, so it is not used as the SE reference.

For M3, `bloq_m3_analytic_covariance_at_nonmem_minimum` evaluates the analytical
FOCEI R-matrix at the stored NONMEM 7.5.1 LAPLACE M3 minimum. Fixed-effect SEs
agree within 0.38%; residual and random-effect variance SEs differ by 5–16%,
consistent with ferx's documented FOCEI Gauss–Newton versus NONMEM full-Laplace
curvature difference. The regression allows 1% for fixed effects and 20% for
variance parameters.

The shared FOCEI covariance machinery is checked against the stored NONMEM 7.5.1
`MATRIX=R` reference at the same parameter values, using
`tests/covariance_se_at_nonmem_minimum.rs`. All seven checked standard errors pass
its 6% acceptance band. This anchors FOCEI; the AGQ Hessian is checked against its
own quadrature objective:

- **Finite-difference oracles at three and five nodes** — diagonal and correlated Ω,
  proportional and combined residual errors. The packed Hessian is compared against
  a reconverged second difference of the AGQ objective in optimizer space. This is the
  check that matters, because it sees the assembly end to end, including the `√2` node
  scaling and the softmax-response term.
- **Reduction to FOCEI at one node** — at `n_agq = 1` the grid collapses onto the mode
  and its node-curvature term reproduces the FOCEI term; the log-determinant still
  includes the quadrature proposal jitter. This check
  *cannot* see a `√2` error (the single node sits at `z = 0`, so the scaling drops out),
  which is why the multi-node oracle above is the primary evidence rather than a
  supplement.
- **Production covariance dispatch** — FOCE, FOCEI, and AGQ-FOCEI standard errors are
  compared against the finite-difference covariance. The test also asserts that the
  analytic dispatcher accepts each fixture and rejects exact-anchor Laplace.
- **Censoring combinations** — FOCE and FOCEI M3 Hessians match reconverged analytic-gradient
  differences, including two-occasion IOV. AGQ-FOCEI M3 matches a reconverged second
  difference of its quadrature objective, both with and without IOV.
- **Anchor identity** — the covariance uses the directly assembled objective anchor,
  without recovering it by inverting its inverse. A regression checks exact matrix
  equality and confirms that the old inverse round trip changes the fixture's entries.

The derivative assembly uses finite differences of the second-order sensitivity jet
for third-order sensitivities. Its node sigma score also centrally differences the
residual variance at fixed predictions; the variance is quadratic in sigma within this
scope, so that step has rounding error but no truncation error.
...
- Models outside the `Dual2` provider take a finite-differenced θ/σ score rather than
  ...
  per iteration.
- Mixture models support plain FOCEI, but `focei` with `n_agq > 1` is rejected:
  the mixture engine does not yet integrate each class by quadrature. It must not
  silently ignore the node count.

## Method boundaries

`focei` with multiple nodes and `laplace` with multiple nodes are intentional
quadrature variants. The former uses the Gauss–Newton anchor; the latter uses the
exact conditional Hessian. A numerical fallback changes how a derivative is
computed, never which anchor or likelihood is requested.
Both file-based and Rust API fits reject `n_agq = 0`; zero cannot silently select
plain FOCEI or one-node Laplace.
The IOV grid cap inspects every stage in a method chain, including a quadrature
stage following a non-quadrature estimator.

An explicitly requested terminal `agq_eval_only` stage is a likelihood readout:
it preserves the preceding estimator's parameters and covariance and updates the
reported OFV. The reported estimation method remains that preceding estimator.

This applies to standard errors too. The R-matrix, the score matrix used by `s`
and `rsr`, and SIR all evaluate the selected quadrature objective. The score matrix
uses subject negative-log-likelihood scores directly; the optimizer doubles these
for its OFV gradient. A FOCEI eta-response correction must not be
added to a quadrature score that already includes mode and grid movement.

If a required derivative perturbation cannot produce a finite anchor or grid
response, only that subject's analytic score is declined first. Its numerical fallback
reconverges the mode and differences the same quadrature objective; other subjects
keep their analytic scores. For `s` and `rsr`, a failed inner solve, rejected start,
or non-finite/penalty objective makes the score unavailable and the covariance step
reports unavailable standard errors. It never squares an optimizer penalty into S,
regardless of `max_unconverged_frac`, and uses `cov_inner_tol` for numerical scores.
Finite-grid posterior-mean scores are
generally nonzero; the exact-integral Bartlett identity does not remove their
mode-response contribution.

The analytic R-matrix is assembled in parallel and summed in fixed subject order.
Zero-weight nodes are skipped before sensitivity evaluation. Grid abscissae are
materialized only for covariance; objective and gradient sweeps reconstruct their
indices rather than allocating another coordinate vector per node.
