...
By default the covariance step recovers standard errors by finite-differencing the
objective function. Under AGQ that is expensive in a specific way: the stencil needs
`~2·n_free²` **reconverged population objectives**, and every one of them sweeps the
whole `n_agq^d` node grid for every subject. Doubling the node count doubles the cost
of each of those `O(n_free²)` evaluations.
...
There is no NONMEM comparison for this feature, because NONMEM has no analytic
covariance for adaptive quadrature to compare against. It is instead validated against
the objective itself:

- **A finite-difference oracle at `n_agq = 3`** — the packed Hessian is compared against
...
- **Reduction to FOCEI at one node** — at `n_agq = 1` the grid collapses onto the mode
  and the assembly must reproduce the FOCE/FOCEI covariance exactly. Note this check
