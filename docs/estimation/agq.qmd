...
uses negative-log-likelihood scores, so optimizer OFV gradients are divided by two
before taking their outer products. A FOCEI eta-response correction must not be
...
response, the analytic-gradient attempt is declined as a whole. The fallback
reconverges the modes and differences the same quadrature objective rather than
returning a gradient with missing terms. Finite-grid posterior-mean scores are
