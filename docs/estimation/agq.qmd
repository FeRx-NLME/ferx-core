...
- **`laplace`, `n_agq = 1` (exact `H`)** — needs the full third-order jet, so each
  ...
  a **diagonal** Ω the two costs are equal by construction (`n_free = n_theta + d`
  either way), which is why successive benchmark sessions on that fixture measured
  opposite signs. The **block-Ω** case is where the call-count gap is real — 20 FD
  rebuilds against 13 provider evaluations — and it measured −24% provider time on
  5 of 5 reps at an identical outer-iteration count and identical OFV. That made it
  the default too.
