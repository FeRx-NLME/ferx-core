...
The two can disagree. A subject whose *data shape* falls outside the provider's scope — a rate-defined infusion under `F ≠ 1`, an out-of-scope ODE dose event — is salvaged individually onto a reconverged finite-difference gradient, while the model as a whole stays in scope and keeps reporting `analytic (Dual2)`. The results are correct; the fit is just slower on those subjects. Since #1154 that mismatch is stated rather than left to be inferred from the run time:

```
1 of 32 subjects fall outside the analytic sensitivity provider's scope (e.g. subject 7)
and use finite-difference outer gradients, although the model itself is in scope; their
results are correct but slower. The reported outer gradient method is the model-level
route, not the per-subject one.
```

The warning fires whenever at least one subject declines **and** the model-level report says analytic — including when *every* subject declines, which is the case the label gets most wrong. A model that already reports finite differences has nothing to reconcile and stays silent.

It counts declines by the sensitivity provider, which are a property of the subject's records. It does not count the outer *assembly* declining at a point away from the EBE — that is a property of where the objective is evaluated, not of the subject, and it resolves itself at the mode the optimizer actually uses.
