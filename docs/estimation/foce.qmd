...
### Exact analytic Hessian (default)

For in-scope analytical-PK models, ferx builds the R-matrix Hessian `H` in **closed form** rather than by finite differences (issue [#436](https://github.com/FeRx-NLME/ferx-core/issues/436)). This is the exact second derivative of the per-subject objective, assembled from the same analytic sensitivities the gradient uses, extended one order:

- **FOCEI** — the Almquist `M2 + M3` decomposition. Writing `Fᵢ = Φ + ½log|H̃|`, the envelope theorem collapses the `Φ` part (M2) to a Schur complement `∂²Φ/∂ξ∂ζ − M_ξᵀH⁻¹M_ζ`, and the `½log|H̃|` part (M3) is its Hessian carried through the moving mode `η̂(ζ)`, using the third-order sensitivities `∂³f/∂η³`, `∂³f/∂η²∂θ`, `∂³f/∂η∂θ²`.
- **FOCE** — the second derivative of the Sheiner–Beal marginal `½[ρᵀR̃⁻¹ρ + log|R̃|]` (the standard Gaussian-marginal / REML form), including the EBE-response curvature. The posterior mode is shared with FOCEI, so the mode-response machinery is identical.

Because it is finite-difference-free, the analytic Hessian carries **no `fd_hessian_step` to tune and no catastrophic cancellation** on flat directions (the source of the inflated `SE(ω²KA)` in issue [#432](https://github.com/FeRx-NLME/ferx-core/issues/432)). At NONMEM's own block-Ω warfarin minimum it reproduces `$COV MATRIX=R` to 5–6 significant figures while running ~20× faster for FOCEI (~9× for FOCE) than the finite-difference covariance.

It is on by default (`analytic_cov_hessian = true`). Any out-of-scope subject — ODE models, output scaling, LTBS, IOV, residual-error random effects, BLOQ censoring, time-varying covariates, oral-infusion, resets, or non-analytical PK — transparently drops the whole population back to the finite-difference covariance described next, so those models are unaffected. Set `analytic_cov_hessian = false` to force the finite-difference path.

### How the finite-difference Hessian is built

When the analytic path does not apply (or is disabled), the covariance is `2·H⁻¹`, where `H` is the finite-difference Hessian of the objective. Two details make the SEs match NONMEM's `$COVARIANCE`:
