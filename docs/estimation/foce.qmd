...
Its scope is narrow, and deliberately so: analytical (closed-form) Gaussian models, including IOV and M3 censoring, without LTBS, `[scaling]`, Form-C readouts, `[initial_conditions]`, `iiv_on_ruv`, correlated or custom-magnitude residual error, FREM, a covariate-`Selected` error spec, or a non-Gaussian endpoint — and not under `method = laplace`, whose marginal needs fourth-order prediction derivatives. FOCE uses the linearized-marginal M3 tail described above; FOCEI uses its conditional M3 tail. FOCEI-anchored AGQ has its own [analytic assembly](agq.qmd#the-analytic-covariance), including IOV and M3. IOV with modeled lag is also excluded. The decision is **all-or-nothing across the population**: one out-of-scope subject returns the whole fit to the finite-difference stencil, because a Hessian half-assembled from two different approximations would be neither. Everything outside the scope keeps the stencil below, subject to numerical convergence and step-size limits.
