...
It is run **twice**: once cold, and once from the EBEs the optimizer was minimising against.
Whichever scores the lower objective is reported. Before
[#833](https://github.com/FeRx-NLME/ferx-core/issues/833) only the cold solve ran, and on a
multimodal individual objective it could settle at a different η̂ than the warm trajectory had —
so the reported OFV came out *above* the best-seen value the point had just been restored for
(+3.5 on a fluconazole 2-cpt binding model), and the covariance step ran at those different
EBEs. Re-solving from both and keeping the better one cannot make the reported number worse
than either.

The two solves are also a diagnostic. A cold re-solve that lands materially worse raises the
`ebe_start_dependent` warning (see [Warnings](../warnings.qmd#codes-convergence)), which quotes
both values: the estimates are fine, but the EBEs at them — and every EBE-derived diagnostic —
depend on where the inner loop starts, which is worth knowing before reading an IWRES plot.
