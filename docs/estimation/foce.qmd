...
| SS + non-autonomous RHS (reads `TIME`/`TAFD`/`TAD`) | ODE | both | |
| `TAD` built-in + estimated lagtime | ODE | both | `∂TAD/∂ALAG = −1` is missing from the analytic chain (#1070) — see below |
...
deliberate FD exception and as closed by #486). The prose below and in
[dose events](#analytic-scope-dose-events) carries both statements as written.

The entries with no home in the scope sections above, in full:
...
Separately, an `[odes]` RHS that reads the **`TAD` built-in while the model carries an estimated lagtime** routes to FD on both loops, and under IOV (#1070): `TAD` is injected into the RHS as a constant, but its anchor is the dose's *lagged arrival*, so the `∂TAD/∂ALAG = −1` term is missing from the analytic chain everywhere the trajectory is integrated. Predictions are unaffected — only the gradient — but FOCEI builds its `h` matrix from that gradient, so the error reaches the reported OFV (2.8%–70% on the lag axis; 0.176 OFV against NONMEM, versus 0.0003 on the FD route). This is scoped as tightly as the defect: `TAFD` anchors at the *unlagged* first dose time and a per-route absorption lag does not move the `TAD` anchor at all, so neither triggers the fallback, and a `TAD`-reading RHS with no lagtime keeps its analytic route. A lagtime that is a literal constant carries no derivative and is likewise exact, but is currently declined with the rest — correct, just slower.

### Moving boundaries and one-sided derivatives {#moving-boundaries}

A moving boundary — a lagged dose arrival, the rate-on of a lagged infusion, an infusion or zero-order window end — is **not a data record**, so it supplies no parameters of its own: it subdivides the interval that the next record terminates, and every piece of that interval runs on that record's snapshot (#1073). A boundary falling *strictly inside* a record interval therefore has the same covariate snapshot on both sides, and the covariate field jump that used to be injected there is simply absent. Any forcing that straddles the instant — another dose's infusion, a zero-order window, a built-in `R_in` — is still carried in both velocities, so the rate and mass jumps stay exact. A genuine two-sided field jump survives only when a **record lands exactly on the boundary**; there the derivative really is two-sided (the value stays continuous, but the field assignment flips) and the walk returns the one-sided derivative of the branch its own event ordering uses rather than declining (#1068). Two narrower approximations in the same cell — a second moving boundary colliding with the arrival (#1076), and a straddling built-in `R_in` whose kernel reads the changing covariate leaving an explicit `½(∂f⁻/∂t − ∂f⁺/∂t)` curvature term uncarried (#1075), which affects the FOCEI Hessian and the covariance step but not the objective or the gradient — are tracked in the #1078 epic; both are reachable only through that same co-timed geometry now.
