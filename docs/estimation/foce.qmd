...
`run_covariance` reconstructs the fitted parameters from the `FitResult`, re-runs the inner loop (warm-started from the per-subject EBEs in `fit.subjects`) to rebuild the covariance-step inputs, and calls the same FD-Hessian covariance step `fit()` runs inline — so the result is numerically identical to fitting with `covariance = true`. The returned `FitResult` is a clone of `fit` with `covariance_matrix`, `se_theta` / `se_omega` / `se_sigma` / `se_kappa`, `covariance_status`, and the condition-number diagnostics refreshed. `run_covariance_step` on the passed options is ignored — calling the function is itself the request.
