...
The provider is not the only thing that can decline. The outer *assembly* inverts each subject's exact inner Hessian $H = \partial^2 l_i/\partial\eta^2$ to form $\mathrm{d}\hat\eta/\mathrm{d}\zeta = -H^{-1}M$, and that quantity needs $H$ **nonsingular** — nothing stronger. Until #1513 it was inverted through a Cholesky, which tests positive-definiteness instead. $H$ is PD at the EBE but carries the residual $\times$ prediction-curvature term that the Gauss-Newton $\tilde H$ drops, so away from the mode it is routinely indefinite: 4 of the 10 subjects on the bundled `examples/warfarin.ferx` fail that Cholesky at $\eta = 0$. Every one of those was a decline, and each decline bought a reconverged finite-difference salvage — $2 \cdot n_\text{free}$ warm EBE re-solves for one subject. An indefinite $H$ now falls back to a full-pivot LU, which is exact wherever the identity holds; the Cholesky stays the fast path, so a fit with no declines is unchanged bit for bit, and a genuinely singular $H$ still declines. On the bundled `examples/one_cpt_transit.ferx` (100 subjects) this was the entire reported fallback — 16 of 100 subjects, gone, with the fit reaching the same optimum in 70 rather than 82 iterations ($\Delta$OFV 2.1e-5) — and on a 55-subject parent$\to$metabolite ODE model where 21 subjects declined, the fit ran 4.1$\times$ faster and its covariance step 136$\times$ faster.

One thing this does **not** fix, and it is worth knowing before reading a fast fit as a correct one. $-H^{-1}M$ also assumes the inner stationarity condition $\partial l/\partial\eta|_{\hat\eta} = 0$, and a subject whose $H$ is indefinite is by construction not at a minimum — so wherever the LU fallback fires, that precondition is generally violated too. Most of those points are blown-up line-search trials that no method scores meaningfully, and the finite-difference salvage they used to take was measured to be no better (the two disagree, and the disagreement tracks $\|\partial l/\partial\eta\|$ subject by subject). The argument for the fallback is that it is exact where the identity holds and cheap where it is not, not that it rescues a broken precondition; the finiteness backstop on each subject's assembled gradient remains what guards the outer optimizer.
