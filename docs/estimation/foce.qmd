...
The two can disagree. A subject the provider declines — a rate-defined infusion under `F ≠ 1`, an out-of-scope ODE dose event, two modeled infusion windows that happen to coincide at the current estimates — is salvaged individually onto a reconverged finite-difference gradient, while the model as a whole stays in scope and keeps reporting `analytic (Dual2)`. The results are correct; the fit is just slower on those subjects. Since #1154 that mismatch is stated rather than left to be inferred from the run time:

```
1 of 32 subjects fell outside the analytic sensitivity provider's scope during this fit
(e.g. subject 7) and used finite-difference outer gradients; their results are correct
but slower. The reported outer gradient method is the model-level route, not the
per-subject one.
```
...
Recording also means the warning is self-gating. A derivative-free BOBYQA fit (including a `[mixture]` model left on `optimizer = auto`, which resolves to BOBYQA), a Gauss-Newton or trust-region fit, a `reconverge_gradient_interval = 1` fit that forces the reconverged gradient on every evaluation, and an `outer_maxiter = 0` evaluation-only run never select the analytic branch, so they have nothing to record and say nothing. Conversely, any recorded decline is by construction a mismatch with the model-level label — including when *every* subject declines, which is the case that label gets most wrong.

The provider is not the only thing that can decline. The outer *assembly* inverts each subject's exact inner Hessian $H = \partial^2 l_i/\partial\eta^2$ to form $\mathrm{d}\hat\eta/\mathrm{d}\zeta = -H^{-1}M$, and it does so with a Cholesky — so a subject whose $H$ is not positive definite declines there too, and takes the same reconverged finite-difference salvage. This is common away from the mode: $H$ carries the residual $\times$ prediction-curvature term that the Gauss-Newton $\tilde H$ drops, and 4 of the 10 subjects on the bundled `examples/warfarin.ferx` fail that Cholesky at $\eta = 0$ while all 10 are positive definite at their EBEs.
...
The practical reading for a user: a `gradient:` banner of `analytic (Dual2)` together with a per-subject fallback warning means some subjects' inner loops are finishing away from their optimum, not that ferx is being needlessly cautious. Those fits are correct and slower.
...
What the missing term can touch was answered per optimizer, from the sources, before a policy was picked. Every gradient-based outer optimizer rejects a point worse than its incumbent on sufficient decrease alone, so nothing served at such a point becomes an accepted iterate or a curvature pair. Beyond that:

| outer optimizer | gradient at a rejected trial point |
|---|---|
| NLopt SLSQP | requested only at the first trial of a line search (`mode = −2`) and discarded when that trial fails the L1-merit test; later trials request no gradient at all |
| NLopt MMA | requested at every inner candidate, copied only on acceptance |
| built-in BFGS / L-BFGS | never requested: the backtracking line search evaluates the objective alone, so the guard is unreachable there |
| Gauss-Newton, trust region | never reach the per-subject assembly |
| NLopt L-BFGS (Luksan) | requested at every trial and **read**: `pnint1` interpolates the next step length from the rejected trial's directional derivative as well as its value |

So for SLSQP and MMA the guard is free, and measured as such: across the 10 bundled examples with skipped salvages under SLSQP every estimate, standard error, OFV, iteration count and sdtab is byte-identical to the unguarded run. For NLopt L-BFGS it is a trade, and the trade decided the policy. The issue ranked the fixed-EBE gradient first as the stand-in, but at the instrumented blown-up events that gradient has norm $10^3$–$10^4$ against 60–180 for the reconverged one — the profiled objective is far flatter there, because the re-solved EBEs absorb most of the blow-up — so dropping the subject's term is the closer stand-in for what the salvage would have returned. Measured on the three L-BFGS examples where the guard fires: with the fixed-EBE stand-in `warfarin_if` went from 51 to 68 evaluations and `two_cpt_oral_cov` from 75 to 79; with the term dropped, `two_cpt_oral_cov` and `warfarin_ode_lagtime` are byte-identical to the unguarded run and `warfarin_if` takes 58 evaluations to the same optimum, estimates agreeing to $6 \times 10^{-6}$ relative. The bundled reproducer `examples/one_cpt_transit.ferx` + `data/datsim_oral.csv` is unchanged to the byte: after #1519 its 15 remaining declines all sit at one trial whose excess over the incumbent is 33 units on 1,200 observations, and its subjects within 1.6 units of their incumbent values — nothing there is blown up, so nothing is skipped.
