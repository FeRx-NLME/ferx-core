...
NLopt's `MaxEvalReached`, not a `Failure`, and a run that stalls on its last permitted
evaluation is not restarted either: the restart runs on the *remaining* budget, so the two
legs together never exceed the `maxiter` you set — raise it instead
([#1428](https://github.com/FeRx-NLME/ferx-core/issues/1428)). A **cancelled** run is not
