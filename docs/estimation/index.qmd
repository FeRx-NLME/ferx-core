...
The outer loop optimizer is selected independently of the method. See **[Outer Optimizers](optimizers.qmd)** for a full comparison of BOBYQA, SLSQP, trust-region, BFGS, and the others. Short version: keep the default (`auto`, which picks `nlopt_lbfgs` when the exact analytic gradient is available and the derivative-free `bobyqa` when only finite differences are — see [the `auto` default](optimizers.qmd#the-auto-default) for the exceptions); name a specific optimizer only to override that choice.
