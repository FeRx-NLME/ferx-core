...
It reports only coordinates the box walk above does not already claim, which
makes the two complementary rather than two opinions on the same coordinate. A
coordinate both would claim is moved *twice* — `sigma PROP_ERR ~ 0.0 (sd)` is
floored to an SD of $10^{-10}$ and then clamped onto the $e^{-8} = 3.355\times
10^{-4}$ rail — so the box message, which names the value the fit actually uses,
is the more useful one and this check stands down. What is left over is
precisely the set nothing else can see: a **`FIX`ed** coordinate, whose box is
pinned to the moved value; a Theta whose declared lower bound is itself at or
below the floor, where value and bound are floored identically; and a free
correlation, which lands exactly *on* its rail rather than outside it.
