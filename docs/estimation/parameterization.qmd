...
- **Outside one of the internal guards above** — the `1e-10` / `1e9` Theta caps,
  the Omega rails, the Sigma rails — is a `W_INIT_OUTSIDE_BOUNDS` warning
...
- An **evaluation-only** run (`maxiter = 0`) is exempt, exactly as for
  `E_OMEGA_INIT_AT_RAIL`. It clamps too; it just does not hold the clamped value
  through a search, and that stickiness is what the check is about.
