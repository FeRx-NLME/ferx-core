...
Concretely, on `examples/one_cpt_iv.ferx` with its omegas removed and its own
`sigma PROP_ERR ~ 0.02 (sd)` start, `gn` begins at OFV 11374 and reports
`Converged: NO`. From `0.13 (sd)` it converges to the optimum. FOCEI reaches the
optimum from either start. If a fixed-effects GN fit stalls, check `sigma` before
anything else — or use `gn_hybrid`, whose FOCEI polish recovers it.
