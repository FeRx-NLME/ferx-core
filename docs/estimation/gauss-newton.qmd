...
2. **FOCEI polish**: Run up to 100 iterations of standard FOCEI optimization, warm-started from the GN result. The polish stage uses the configured outer optimizer (`optimizer` in `[fit_options]`), which defaults to `auto` (L-BFGS with an analytic gradient, BOBYQA otherwise) — see [Outer Optimizers](optimizers.qmd) for the applicability matrix.
