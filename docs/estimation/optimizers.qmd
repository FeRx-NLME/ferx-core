...
For a FOCE/FOCEI fit the resolved optimizer is recorded in the fit output as `auto (<resolved>)` — e.g. `auto (nlopt_lbfgs)` — so a run records which algorithm actually executed. Two exceptions: `gn_hybrid` reports `gn` even though its FOCEI polish runs the resolved optimizer, and a mixture model's label currently ignores the mixture override, so it can read `auto (nlopt_lbfgs)` while BOBYQA ran ([#1540](https://github.com/FeRx-NLME/ferx-core/issues/1540)). These defaults may be refined as the analytic-gradient scope grows and more datasets are studied; pin `optimizer` explicitly to opt out of the automatic choice.
