...
one the unforced arm would use. The warning is silent when nothing was coupled:
when `optimizer` is pinned, when the model is out of analytic scope anyway
(`auto` picks `bobyqa` either way), and when the model is large enough that
`auto` takes `nlopt_lbfgs` on a finite-difference gradient regardless.
