...
If the fit nevertheless ends up still on its initial estimates, ferx re-runs it once from the same start with the rescale **held on until the fit escapes** — a fit that never moved has no curvature worth protecting — and reports whichever of the two attempts reached the lower OFV. A fit that was descending normally never triggers this and follows exactly the trajectory it always did.
