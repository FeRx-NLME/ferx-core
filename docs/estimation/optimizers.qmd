...
### Conditioned (experimental, #864)

| Key | Algorithm | Notes |
|-----|-----------|-------|
| `conditioned` | Preconditioned L-BFGS + strong-Wolfe | **Experimental, opt-in, default-off — a work-in-progress prototype.** One co-designed path for ill-conditioned cold starts: the built-in L-BFGS loop driven with a gradient-based diagonal preconditioner, a strong-Wolfe (curvature-condition) line search, and restart-on-stall. **Not yet recommended for production**: on the current prototype it matches the default on well-conditioned fits but does *not* yet beat it on ill-conditioned models — the coordinate (hybrid `log θ` / `θ/θ_init`) lever is still to land. Selecting it is only useful for evaluating the conditioning rework. |
