...
measurable progress — no objective improvement beyond `outer_ftol` (default
`1e-6`, relative) and no parameter movement beyond `outer_xtol` (default `1e-4`,
relative). A run that instead exhausts `maxiter` is reported as **`Converged:
NO`**, with a warning naming the budget it ran out of and the gradient norm it
stopped at.
...
|---|---|---|---|
| 5 | 11325.8283 | `Converged: YES` | `Converged: NO` |
...
The stopping rule is deliberately **not** a gradient test. The outer gradient here
is evaluated at fixed EBEs, which makes it unusable as a stationarity measure on
an ODE model: on `one_cpt_iv_ode` the gradient norm at the optimum the default
optimizer reaches (OFV −387.0737) is 43.2, while a *worse* point 0.73 OFV units
away carries 4.0 — the norm is not monotone in solution quality, so no threshold
separates the two. Objective and step size are the noise-robust measures, and they
are what the NLopt outer optimizers already stop on.
...
slightly above the optimum other optimizers reach (the `one_cpt_iv_ode` case
above, and [#864](https://github.com/FeRx-NLME/ferx-core/issues/864)). Compare
against a second optimizer when the OFV matters.
