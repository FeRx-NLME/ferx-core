...
- A **non-identified direction** comes back from the covariance step with an
  ...
  each direction so ±2 proposal standard deviations still fit inside the packed
  bounds, and warns that CIs along those directions understate the uncertainty.
  The cap is deliberately loose — it bites only a direction whose proposal is
  wider than the box the parameter is declared to live in — so a legitimately
  wide uncertainty is left alone.
...
bounds [TVKA -0.74, ETA_KA -0.60, ETA_V +0.28 (sd 6.15e3 → 2.89e0); ETA_V +0.84,
