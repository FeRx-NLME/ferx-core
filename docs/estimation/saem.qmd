...
θ/σ. Run anyway, it terminates on its own criterion slightly short of the
...
fails up front rather than after the SAEM stage has already run. `imp`, `impmap`
and `bayes` reject `n_eta = 0` for the same reason.
