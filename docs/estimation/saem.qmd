...
when NLopt is left estimating a theta that carries **no ETA**, which is exactly
the situation the advisory above warns about. A theta that is `FIX`, that is
pinned out by the mu-reference shift, or that simply carries an ETA (so the eta
absorbs the per-draw misfit) is left on the undamped update, and a fit whose
every estimated theta is one of those is unaffected down to the last bit — and
setting `mstep_damping` on such a model warns that it has no effect rather than
