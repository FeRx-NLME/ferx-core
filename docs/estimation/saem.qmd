...
starts local and converges, and the exploration cap is off by default.
...
default** (`1.0`, plain assignment in both phases):
...
(7492.7 against 7445.3), so on this model the hold is the better answer. For that shape of model `mstep_damping = 0.03` remains the
documented way to hold the no-ETA thetas at values estimated elsewhere; `FIX`
is the honest spelling of the same thing.
