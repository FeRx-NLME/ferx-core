...
With `iiv_on_ruv = ETA`, the residual variance is $\sigma^2 \exp(2\eta_{\text{RUV}})$, so the *marginal* residual is a function of $\sigma^2 \exp(2\omega_{\text{RUV}})$: $\sigma$ and $\omega_{\text{RUV}}$ trade off along a ridge and are only jointly identified. If the E-step mixes poorly, the M-step can ride that ridge until $\sigma$ hits its internal e⁵ (≈148 SD) ceiling while $\omega_{\text{RUV}}$ inflates to compensate — the failure reported in issue #895 (against 0.2.0, before the componentwise kernel).

ferx guards this two ways. The componentwise kernel keeps the E-step mixing even for the FREM-scale $\Omega$ that triggered the original report, so the ridge does not run away in practice. As a backstop, each free RUV-scaled residual $\sigma$ is capped during SAEM at $\approx 20\times$ its starting SD (the cap is a post-M-step clamp, so a well-posed fit whose $\sigma$ stays below it is reproduced bit-for-bit). If a $\sigma$ ends pinned against that cap, SAEM warns that the σ × ω_RUV split is weakly identified — fix $\sigma$ (or the RUV $\omega$) at a known value, or drop `iiv_on_ruv`.
