...
**It is opt-in, and not a free win.** On the warfarin example it moves the
chain from 2–4% to ≈42% acceptance and cuts the final-estimate error by ~60%,
and with the scale at target 8 proposals per subject per iteration beat the
default 20. On models whose baseline scale is already near target it changes
little. But on a validation model whose acceptance was already *above* target
(0.45 against 0.40), driving it to target measurably **worsened** the fit —
final-estimate error up 24% and seed-to-seed spread up 68%, with the marginal
likelihood agreeing, so it is not an artifact of the reference. The default is
therefore `interval`, which leaves every existing fit's estimates unchanged.
