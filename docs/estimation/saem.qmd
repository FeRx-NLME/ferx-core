...
Such a theta never receives the γ-damped closed-form shift. Its only channel is
the numerical M-step, which re-maximises the conditional observation likelihood
against a **single** MCMC η draw with no stochastic-approximation damping. That
is a random walk on a noisy surface rather than an SA-averaged sufficient
statistic, and it can drift a long way from the marginal optimum — dragging any
correlated typical value with it.
...
(`D1 = MAT*(1-TVFRD1)`, `KA = 1/(MAT*TVFRD1)`) with no eta of its own:
...
| ferx SAEM | 7.169 | 143.2 | 3.020 | **0.039** |
| ferx SAEM, started *at* the IMPMAP solution | 7.046 | 136.4 | 3.191 | **0.065** |
...
Restarting SAEM at the right answer does not rescue it — the M-step walks the
parameter back down — so this is not a starting-value problem. The two remedies
that do work are the ones the warning names:

* **Attach an eta** — `P = TVP * exp(ETA_P)` with a small omega (optionally
  `FIX`). ferx then mu-references it automatically and the closed-form,
  γ-damped update takes over. Here it recovered `TVFRD1` to 0.313 and removed
  the `TVMAT` bias (+9.2% → −1.3%).
