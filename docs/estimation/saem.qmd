...
with $A_i$ the eta-free typical value at subject $i$'s covariates and $g$ the link (`log` or the logit-scale identity). When every covariate the value reads is **constant within each subject**, the data term does not depend on $\theta$ and the step is an exact nonlinear least-squares fit (Gauss–Newton); when a covariate **varies within a subject**, the within-subject ratio $A_i(t)/A_i(t_0)$ keeps the data term live and the sum is minimised numerically (BOBYQA), still in the $\phi$-frozen coordinates. The new estimate is blended with the same SA step $\gamma_k$ as the single-anchor shift, and each η is re-centred by the change in its own mu so $\phi_i$ is unchanged. This is NONMEM's `MU_1 = LOG(THETA(1) + (CRCL-90)*THETA(2))` — a MU that is nonlinear in THETA — done automatically.
