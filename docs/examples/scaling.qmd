...
- **Response versus time**: a Form C readout may reference the `TIME` built-in (spelled `TIME` or `T`, as in `[odes]`), which resolves to each observation's own time — the shape a disease-progression, placebo-response, or model-based meta-analysis readout takes. See [`examples/scaling_time_readout.ferx`](https://github.com/FeRx-NLME/ferx-core/blob/main/examples/scaling_time_readout.ferx), anchored against NONMEM's `$ERROR` in `nonmem_anchor/scaling_time_readout.ctl`. `obs_scale` cannot use `TIME` — it is a subject-static divisor, evaluated once at `t = 0`, so the parser rejects it and points at Form C.
