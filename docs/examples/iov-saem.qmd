...
- **`n_mh_steps`**: number of block Metropolis-Hastings steps per subject per SAEM iteration. Leave it at the default `auto`, which sizes it from the observations per subject per random effect (6–20; see [Choosing `n_mh_steps`](../estimation/saem.qmd#choosing-n_mh_steps)); the `3` in the model above is this example's own deliberately small setting, kept so the page runs quickly. Raise it above the rule's answer for a model with strong random-effect correlations.
