...
- **SDE + ADDL**: steady-state dosing (`SS=1`) and `ADDL` are supported with the SDE solver. At an `SS=1` record the filter expands the pulse train until both its mean and its covariance stop moving, so the covariance at the record is the numerically equilibrated periodic (Riccati) value rather than a closed-form reset — see [Steady-state records are equilibrated](../model-file/diffusion.qmd#sde-steady-state). An `SS=1` infusion longer than `II` is declined (`W_STEADY_STATE_INFUSION`).
