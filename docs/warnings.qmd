...
| `optimizer_health` | Warning | Trust-radius collapse / degeneracy, or an L-BFGS line-search abort that was [resumed from the best point seen](estimation/optimizers.qmd#lbfgs-abort-resumed). | Try a different optimizer or re-scale parameters. A resumed fit that ends converged needs no action; one that did not is usually short of `outer_maxiter`. |
