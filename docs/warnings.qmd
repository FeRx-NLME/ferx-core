...
honour. `fit()` has always returned these as an `Err`. Since ferx 0.4.0 so does
every other entry point that returns a `Result`, **with the same text** — the
subject, the time and the remedy, and nothing wrapped around them:

| Entry point | On a failed precondition |
|---|---|
| `fit()`, `predict_diag()`, `predict_survival()`, `predict_categorical()`, `inits_from_nca()`, `simulate_with_options()`, `simulate_with_options_diag()`, `simulate_with_uncertainty()`, `simulate_adaptive()` | `Err(message)` |
| `predict()`, `simulate()`, `simulate_with_seed()` | panic whose payload is that same `message` — they return a bare `Vec` and have no error channel. Call the `Result` form above them to handle the failure instead. |
