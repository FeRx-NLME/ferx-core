...
| `ode_solver` | Warning / Info | The ODE solver's own health at the final estimates (#1080). **Warning** when steps clamped at the minimum step size (a stability-limited segment whose un-integrated tail is freeze-padded with the last state), when a segment stopped before its requested end, when `ode_method = auto` escalated a segment to a stiff method and that escalation had to be discarded and re-solved explicitly, when that explicit fallback also failed, or when `ode_stiff_abort_after` cut segments short. **Info** when `auto` escalated and everything worked (a separate `W_ODE_SOLVER_ESCALATION_NOTE` message token, so the severity survives re-classification from the plain text) — routine on a stiff model, but a decision you never asked for and could not otherwise see. Both severities report how many segments changed stepper **part-way through** (`ode_auto_switch`, on by default), because the steps before such a switch were taken by a different method than the ones after it. | A rejected escalation means the stiffness probe was right and its method choice wrong: name `ode_method = rodas5p` (or `rosenbrock23`) explicitly. If `auto_fallback_failed` is non-zero, neither attempt produced a usable trajectory; do not trust the fit until a named method and appropriate tolerances integrate cleanly. Clamps with no rejection usually mean the explicit stepper is stability-limited — try `ode_method = auto` / a stiff method, or loosen `ode_reltol` / `ode_abstol`. |
...
| `ode_solver` | `phase` (`"postfit_predictions"`), `ode_method`, `attempted_steps`, `accepted_steps`, `rejected_steps`, `min_step_clamped_steps`, `stiff_min_step_clamped_steps`, `discarded_clamped_steps`, `kept_clamped_steps`, `auto_stiff_segments`, `auto_switched_segments`, `auto_stiff_rejected`, `auto_fallback_failed`, `unfinished_segments`, `discarded_unfinished_segments`, `kept_unfinished_segments`, `stiff_aborted_segments`. These are not all disjoint: `kept_unfinished_segments` is a roll-up that already includes `stiff_aborted_segments`, and `auto_fallback_failed` ⊆ `auto_stiff_rejected` ⊆ `auto_stiff_segments`. The message text subtracts the overlaps before reporting; a consumer reading the payload should not add these keys together. |
