...
| Entry point | Preconditions it checks |
|---|---|
| `fit()`, `ferx check` | all of them |
...
| `simulate_with_options()`, `simulate_with_options_diag()`, `simulate_with_uncertainty()` | the simulation data checks (covariates present, `[covariate_model]` bound and levels, endpoint routing, κ and residual weights), the TTE horizon, Ω~IOV~ present, then coded `RATE`, dose compartment, absorption closed-form support, flip-flop without an ODE twin, time-varying covariate on a hazard, absorption input rate, a CTMM model. **Not** the depot readout across a reset. |
...
| `simulate_adaptive()` | coded `RATE`, dose compartment, absorption closed-form support, absorption input rate, plus its own adaptive-dosing rejections. **Not** the time-varying covariate on a hazard. |
| `predict()`, `simulate()`, `simulate_with_seed()` | the list of the `Result` form each wraps — but the failure is a **panic** whose payload is that same message, because they return a bare `Vec` and have no error channel. Call the `Result` form to handle it instead. |
