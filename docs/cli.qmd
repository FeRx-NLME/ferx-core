...
to fit. A tool name is a bare word, so anything carrying a `.` or a path
separator is read as a file path — and a file that exists is always taken as
the model, whatever it is called.

That split decides which error you get when the argument resolves to nothing:
