...
Pharmpy's `modelsearch`: a search over the analytic `pk` templates — absorption
route, peripheral compartments, transit compartments and lag time — driven by
a [`.ferxsearch` file](tools/search.qmd) whose `ABSORPTION`, `PERIPHERALS`,
`TRANSITS` and `LAGTIME` statements are the space and whose `[modelsearch]`
