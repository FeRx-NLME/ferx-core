...
from a sample time. ferx applies a single tolerance, `1e-12`, and applies it
**one-sided**:

| separation | read |
|---|---|
| arrival **at or up to `1e-12` before** the sample | post-dose |
| arrival **after** the sample, by any amount | pre-dose |

A sample inside that band is read from the state just after the arrival's events
(reset, then dose), rather than by integrating the sub-picosecond span to the
sample's own time; the difference is bounded by `|f|·1e-12`, far below any solver
tolerance. The one-sidedness is the part worth knowing: an arrival even one ULP
*after* a sample has not happened yet at that sample, and never contributes to it.
