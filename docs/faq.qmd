...
differences — ODE/PD models, LTBS, SDE, or `gradient_method = fd`. Mixture
models also resolve `auto` to `bobyqa`. The fit output reports which
optimizer `auto` chose; see
[Outer Optimizers](estimation/optimizers.qmd#the-auto-default) for the rules
and the benchmarks behind them. To pin one, set `optimizer = ...` in
`[fit_options]`.
