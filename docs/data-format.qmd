...
| `CMT` | integer | 1 | Compartment number (1-indexed). A float-formatted integer (`2.0`, as pandas and R write a whole column once any cell in it is blank) reads as that integer. A missing (`.`/blank/`NA`) or unreadable cell, and an absent column, fall back to compartment 1 — reported as `W_CMT_DEFAULTED` when the model addresses more than one state; see [`CMT` on `[odes]` models](#cmt-on-odes-models). |
...
`CMT` addresses **two** things, and the warning covers both. The first is which compartment a dose lands in, which only a multi-state `[odes]` model can get wrong. The second is which readout, scale or error model an *observation* uses — and that one exists on the analytical engine too: a per-CMT `[scaling]` block parses on a `pk` model, and its validation only checks that the *observed* compartments have entries, so a `CMT`-less dataset keys every row to 1, `{1}` is a subset of `{1, 2}`, and nothing complains. Measured on `pk one_cpt_iv` with `obs_scale[CMT=1] = 1000` and `obs_scale[CMT=2] = 1`: the same data spelled with `CMT=2` gives OFV 0.0357, with the column dropped 6097015712.1246. The warning is silent only for a model where `CMT` selects nothing — a plain analytical `pk` model, a one-state `[odes]` model, or a compartment-free model.
