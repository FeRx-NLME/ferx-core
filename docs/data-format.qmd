...
|--------|---------|---------|
| `TIME`, `EVID` | a number; a non-negative whole number for `EVID` | every record |
...
- **A record the `[data_selection]` block removes is not read**, so a cell on it
  ...
  removes the record must not read the unreadable column itself: `ignore = CENS
  == 0` on `CENS = abc` would decide the record on a value the dataset does not
  hold, so it is an error too, naming the rule. The same holds for a rule that
  reads the column and keeps the record. NONMEM 7.5.1 agrees on both sides:
