...
Declared survival and discrete endpoints are the deliberate exception. A `CMT`-less dataset leaves them with no rows at all, which `E_ENDPOINT_NO_RECORDS` already reports as an error naming the missing column — a better diagnostic than a warning about a guessed compartment.
