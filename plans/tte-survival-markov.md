...
(declining-hazard γ<0 Gompertz sampler) filed → PR #804 in flight; §19 Track-A row + header NEXT
...
| **A · Survival hardening** | ✅ #741/#762/#763/#764 (#772), #770 (#792), #740 left-trunc (#799/#801); **remaining:** #803 sampler (PR #804 in flight), #626 perf (not urgent), Phase 3.4 docs (low value), Phase 2 deferred (IntervalCensored+ODE, left-trunc+ODE), ferx-r `predict_survival` + e2e + ferx-r#210 | trunk | ✅ **correctness done** (pending #804) | `survival` |
...
are all merged; the one remaining correctness item — #803 (declining-hazard γ<0 Gompertz sampler
censoring every draw, simulate ≠ fit) — is in flight as **PR #804**. Track A's residue is non-blocking:
