...
- **`model = NAME` names the model.** Only the bare `model NAME` spelling was read; the `KEY = value` form every other setting uses was dropped in silence and the fit, `-fit.yaml` and ferx-r fell back to the file stem. Both spellings now name the model, a `model` line the parser cannot read (or a second one) is an error naming the accepted forms, only the preamble before the first `[block]` header is read, and `ferx check --json` reports the same name the fit will carry instead of always the stem ([#1395](https://github.com/FeRx-NLME/ferx-core/issues/1395)).
