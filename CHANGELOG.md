...
- FO/FOCEI inner optimization now seeds dense BFGS from the current analytical
  ...
  quadrature objective. Laplace skips generic one-node grid construction, while
  AGQ caches log weights and stores retained node modes contiguously to reduce
  allocator pressure. Unsupported or non-positive-definite seeds retain the
  previous initialization ([#1389](https://github.com/FeRx-NLME/ferx-core/pull/1389)).
