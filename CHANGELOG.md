...
- **An L-BFGS line-search abort no longer ends the fit.** NLopt's L-BFGS returns a bare `Failure` — discarding every point it accepted on the way — whenever one line search exhausts its ten reductions or ten extrapolations at an iteration where it has just restarted, which the first iteration always is. On a model with a large initial gradient the opening-step rescale makes exactly that likely: the reopened #1277 fixture accepted eleven improving evaluations (OFV 18759 → 7975), hit the extrapolation cap, and was reported at that point with `converged = false`, ~9000 OFV units above its optimum. A bare `Failure` that the existing plateau verdict refuses to call converged is now **resumed from the best point seen** — fresh identity Hessian, opening rescale re-armed — on the remaining `outer_maxiter` budget, and the fit carries a warning saying how many times it was resumed. A `Failure` at a genuine plateau is reported converged exactly as before; a resumed launch that makes no progress is put through that same plateau verdict whatever status NLopt returns for it, so a point the verdict rejected cannot come back `converged` on the strength of an `XtolReached`; and a fit that runs out of budget while still descending is reported as a spent budget rather than a stall. See [An L-BFGS abort mid-fit is resumed](https://ferx-nlme.github.io/ferx-core/estimation/optimizers.html#lbfgs-abort-resumed) ([#1277](https://github.com/FeRx-NLME/ferx-core/issues/1277)).
