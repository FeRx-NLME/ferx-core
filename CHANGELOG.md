...
- **A `block_omega` next to a separate diagonal `omega` now keeps the covariances between them at exactly 0, and so does a `block_kappa` next to a separate `kappa`.** Every estimator that runs the outer optimizer — FOCE, FOCEI, Laplace / AGQ, with any `optimizer` including the trust region — searched the cross-block Cholesky entries anyway (Gauss-Newton, SAEM and VI already held them). So `block_omega (ETA_CL, ETA_V)` + `omega ETA_KA` estimated `Cov(ETA_KA, ETA_CL)` and `Cov(ETA_KA, ETA_V)` and returned the full 3×3 block fit — same Ω, same OFV — while `n_parameters` already counted only the declared block. Estimates, OFV and AIC/BIC of any such fit change; `n_parameters` does not. Those entries now report an SE of exactly 0 (the covariance step already excluded them; the estimate itself no longer moves), and SIR and asymptotic uncertainty draws no longer perturb them. For Rust callers, `estimation::parameterization::packed_fixed_mask` now marks these structural zeros as held along with FIX coordinates (#1018).
