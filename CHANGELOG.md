...
- **A fit restarted mid-descent now runs on the remaining `maxiter` budget and says so.** The [#1277](https://github.com/FeRx-NLME/ferx-core/issues/1277) restart gave its second leg a fresh evaluation budget — measured as 56 evaluations on a `maxiter = 1` (44-evaluation) fit — while documenting that it could not; it now continues on what the stalled leg left, a stall on the last permitted evaluation is reported rather than restarted, and an adopted restart is named in `FitResult.warnings` as an `optimizer_health` entry (it was visible only under `verbose`). Fits that are not restarted are bit-identical ([#1428](https://github.com/FeRx-NLME/ferx-core/issues/1428)).
