...
- **A non-finite dose arrival on the analytical superposition engine no longer predicts a drug-free `0.0`.** `predict_concentration` compares each dose's arrival against the observation time, and every comparison against `NaN` is false, so the dose was skipped and the remaining (empty) trajectory was returned as a valid prediction; a `NaN` that did reach the sum was then floored by the trailing `max(0.0)`. Both paths now return `NaN`, which is what the ODE engines have done since [#1189](https://github.com/FeRx-NLME/ferx-core/issues/1189). Reachable from a `NaN` `lagtime` as well as from a modeled `D{n}`/`R{n}` ([#1284](https://github.com/FeRx-NLME/ferx-core/issues/1284)).
