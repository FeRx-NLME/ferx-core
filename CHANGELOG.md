...
- **`W_CMT_DEFAULTED` — a warning naming every dose and observation row whose compartment ferx chose.** A dataset with no `CMT` column, or with cells that are missing or unreadable, gets compartment 1; NONMEM has no such gap, because `$MODEL` lets a model declare `DEFDOSE` on any compartment and NM-TRAN resolves an undecorated dose against it. A translated model whose `DEFDOSE` is not the first declared state therefore gets its drug in the wrong place, silently. The warning gives the dose and observation row counts, the cause (absent column, or how many cells were missing versus unreadable, with examples), and the remedy. The message gives the dose and observation row counts — and, when a defaulted row carried `ADDL`, the expanded dose count, since one cell can deliver thirty-one doses to the guessed compartment. It reaches both `fit()` and `ferx check`, and is raised whenever `CMT` addresses anything: more than one `[odes]` state to dose into, **or** an observation-side dispatcher (`[scaling] obs_scale[CMT=N]`, a `CMT=N:` error model, a per-CMT readout) on either engine. A plain analytical `pk` model with none of those resolves compartment 1 to its own default channel and is bit-identical with and without the column, and a one-state or compartment-free model has no second compartment to have missed ([#1009](https://github.com/FeRx-NLME/ferx-core/issues/1009)).
