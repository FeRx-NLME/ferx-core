...
/// PR #1299 review: on this grid every INST point with the dead KA gene is
/// a cross-batch duplicate of the one without it under the GA, and with
/// seed 1 such a duplicate won — and came back without its fit, so
/// `final-fit.yaml` was skipped and `final.ferx` kept starting values.
/// Whatever wins, its fit is available.
#[test]
fn a_winning_duplicate_still_hands_back_its_fit() {
    let dir = tempfile::tempdir().unwrap();
    let path = write_config(
        dir.path(),
        "ABSORPTION([INST,FO]); COVARIATE?(KA, WT, exp)",
        "[globalsearch]\nalgorithm = \"ga\"\n[globalsearch.ga]\npopulation_size = 3\n\
         generations = 2\nseed = 1\n",
    );
    let result = run(dir.path(), &path, false);
    let winner = result.row(&result.final_id).unwrap();
    assert!(winner.eligible(), "{:?}", result.notes);
    assert!(
        result.final_fit.is_some(),
        "{} (duplicate of {:?}) has no fit; notes: {:?}",
        winner.id,
        winner.duplicate_of,
        result.notes
    );
    assert_eq!(result.final_fit.as_ref().map(|f| f.ofv), winner.ofv);
    assert!(
        !result
            .notes
            .iter()
    ...
    // And the files: the estimates are written, the final model is seeded.
    let yaml = dir.path().join("run/final.ferx");
    assert!(yaml.exists());
    ...
    assert!(
        !seen.is_empty(),
        "the grid produced no duplicate at all: {:?}",
        result.rows.len()
    );
}
