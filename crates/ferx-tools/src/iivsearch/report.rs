//! `models.csv`, `models/<id>.ferx` and `final.ferx` — what a
//! variability-structure search leaves behind (#1183).
//!
//! One row per fitted model — input, base, every candidate of every step —
//! with its structure in Pharmpy's spelling (`[CL,V]+[KA]`), its criterion
//! and Δ against its step's parent, its rank within the step, and beside
//! them the convergence status, the strictness verdict, the starts it was
//! fitted with and the wall-clock seconds the fit took.

use std::path::{Path, PathBuf};

use ferx_core::edit::ModelEdit;

use super::{describe, IivsearchResult, StepKind};
...
pub const MODEL_COLUMNS: [&str; 18] = [
    "id",
    "parent",
    "step",
    "description",
...
pub fn write_report(dir: &Path, result: &IivsearchResult) -> Result<(), String> {
...
fn write_models(path: &Path, result: &IivsearchResult) -> Result<(), String> {
...
/// A human-readable rendering of the search: the input and base, every
/// step's ranking, the final model. What `ferx iivsearch` prints.
pub fn render_summary(result: &IivsearchResult) -> String {
    ...
    if result.base_id != "input" {
        if let Some(base) = result.row(&result.base_id) {
            let _ = writeln!(
                out,
    ...
    for step in &result.steps {
        ...
        for r in &step.ranked {
            ...
            let decision = if r.id == step.best && step.kind != StepKind::Input {
                if r.id == step.parent {
                    "kept".to_string()
                } else {
                    "BEST".to_string()
                }
            } else if step.kind == StepKind::Input && r.id == step.best {
                "SELECTED".to_string()
    ...
    let _ = writeln!(
        ...
        "\nFinal model: {} — {} ({label} {})",
        result.final_id,
        describe(&result.final_structure),
