...
///
/// Varying *within* an occasion is a warning rather than an error: κ is drawn
/// once per occasion, so a weight that changes underneath it makes the arm's
/// effective variance ambiguous — but the model still evaluates, and a dataset
/// whose arm size legitimately changes mid-occasion (a dropout-adjusted N) may
/// be exactly what the author intends.
fn check_kappa_weights(model: &CompiledModel, population: &Population) -> Vec<Diagnostic> {
    ...
    for (k, weight) in model.kappa_weights.iter().enumerate() {
        ...
        let mut warned_variation = false;
        for subj in &population.subjects {
            for (occ, obs_idx) in crate::stats::likelihood::iov_occasion_groups(subj) {
                ...
                for &j in &obs_idx {
                    ...
                    match seen {
                        ...
                        Some(prev)
                            if !warned_variation
                                && (prev - w).abs() > 1e-9 * prev.abs().max(1.0) =>
                        {
                            warned_variation = true;
                            diags.push(
                                Diagnostic::warning(
...
pub fn check_model_data_rule(
    ...
    // Errors only here; the within-occasion-variation *warning* the same walk
    // produces is surfaced by `check_model_data_warnings` so `fit()` reports it
    // too (this list is consumed error-first by `first_error`).
    diags.extend(
        check_kappa_weights(model, population)
            .into_iter()
...
pub fn check_model_data_warnings(
    ...
    // Warning half of `check_kappa_weights` (#1031) — a sample-size weight that
