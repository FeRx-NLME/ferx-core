...
//! | `W_CENS_UNEXPECTED`       | an observation row's `CENS` cell is a value other than `-1` (above ULOQ), `0` (quantified) or `1` (below LLOQ). `m3_logcdf` reads only the flag's sign — a positive flag is the lower tail, like `1`, and a negative one is the upper tail, like `-1` — so under `bloq_method = m3` such a row is scored as censored rather than rejected, and this says so, naming that tail, instead of leaving it silent. Reported once **per sign** per subject, naming the first such value of that sign. A cell that is not a whole number at all (`1.5`, `abc`) is an `E_DATA` error instead — it has no sign to be scored by (#1496) |
