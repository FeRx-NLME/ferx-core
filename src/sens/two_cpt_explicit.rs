...
mod tests {
    ...
    fn two_cpt_iv_bolus_explicit_matches_dual() {
        for &(amt, t, cl, v1, q, v2) in &[
            ...
            (1000.0, 1.0, 4.41, 15.5, 3.17, 29.3), // 2-cpt NONMEM-fit-ish
        ] {
    ...
    fn two_cpt_infusion_explicit_matches_dual() {
        ...
        for &(rate, amt, t, cl, v1, q, v2) in &[
            ...
            (1000.0, 1000.0, 0.5, 4.41, 15.5, 3.17, 29.3), // during (dur=1), fit-ish
            (1000.0, 1000.0, 3.0, 4.41, 15.5, 3.17, 29.3), // after
        ] {
            let dur = amt / rate;
    ...
    fn two_cpt_oral_explicit_matches_dual() {
        ...
        for &(amt, t, cl, v1, q, v2, ka, fb) in &[
            ...
            (1000.0, 8.0, 4.41, 15.5, 3.17, 29.3, 0.6, 1.0), // fit-ish
        ] {
