...
pub fn ode_analytical_supported(model: &CompiledModel) -> bool {
    ...
    // kinds lifted to PkNum: inverse-Gaussian (#430 slice 1), transit (#430
    // slice 2), and Weibull (Phase 2) are all lifted. The check stays
    // kind-agnostic via `supported_over_dual()` so a *future* unlifted kind keeps
    // the FD fallback (a model using it is not "supported" here) without editing
    // this gate.
