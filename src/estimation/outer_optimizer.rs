...
///   `prepare_stacked` needs the true inner Hessian `H` to be invertible, and `H` is PD at
///   the EBE but routinely indefinite elsewhere: measured on the bundled
///   `examples/warfarin.ferx` + `data/warfarin.csv`, 4 of the 10 subjects (ids 2, 4, 7, 10)
///   fail `h_inner.cholesky()` at `η = 0` while the provider serves all 10 and all 10 are
///   PD at their EBEs. Since #1513 that particular shape no longer declines — `−H⁻¹M` only
///   needs `H` nonsingular, so an indefinite `H` takes a full-pivot LU and all 10 warfarin
///   subjects are served at `η = 0` (pinned by
///   `sens_outer_gradient::tests::warfarin_zero_eta_non_pd_subjects_are_served_analytically`).
///   A *singular* `H` still declines, so the asymmetry the bullet describes survives — it is
///   simply much rarer than it was, and a zero-η probe would still get it wrong.
///
