...
/// #1070: a `TAD`-reading `[odes]` RHS under an estimated lagtime declines to FD
/// under IOV, and `iov_fd_reason` must **name** it rather than fall through to the
/// generic "model outside IOV analytic scope".
///
/// That catch-all is the live hazard here, not a hypothetical one: the #1070 decline
/// lives in `ode_iov_supported`, which `iov_sens_eta_supported` calls — so the scope
/// check fires *before* the per-subject ODE attribution block further down, and a
/// clause placed in that block would be unreachable for this cause. This test is
/// what pins the ordering.
#[test]
fn iov_fd_reason_attributes_tad_dependent_rhs_with_lagtime() {
    let model = crate::parser::model_parser::parse_model_string(
        "[parameters]\n  theta TVCL(1.0,0.1,10.0)\n  theta TVV(20.0,1.0,200.0)\n  theta TVLAG(0.4,0.01,5.0)\n  omega ETA_CL ~ 0.09\n  kappa KAPPA_CL ~ 0.01\n  sigma PROP_ERR ~ 0.1 (sd)\n[individual_parameters]\n  CL = TVCL * exp(ETA_CL + KAPPA_CL)\n  V = TVV\n  LAGTIME = TVLAG\n[structural_model]\n  ode(states=[central])\n[odes]\n  d/dt(central) = -(CL/V) * central * (1.0 + 0.3 * TAD)\n[scaling]\n  y = central / V\n[error_model]\n  DV ~ proportional(PROP_ERR)\n[fit_options]\n  method = focei\n  iov_column = OCC\n",
    )
    .expect("parse TAD + lagtime ODE IOV");
    assert!(model.has_lagtime(), "precondition: model carries a lagtime");
    let subject = Subject {
        id: "1".into(),
        doses: vec![DoseEvent::new(0.0, 100.0, 1, 0.0, false, 0.0)],
        obs_times: vec![1.0, 2.0, 5.0, 8.0],
        obs_raw_times: Vec::new(),
        observations: vec![4.0, 3.5, 2.0, 1.0],
        obs_cmts: vec![1; 4],
        covariates: HashMap::new(),
        dose_covariates: Vec::new(),
        obs_covariates: Vec::new(),
    ...
    assert!(
        iov_inner_subject_route(&model, &subject, &model.default_params.theta).is_none(),
        "#1070: TAD-reading RHS + lagtime must route to FD under IOV"
    );
    assert_eq!(
        iov_fd_reason(&model, &subject),
        "TAD-dependent ODE RHS + estimated lagtime"
    );
}
