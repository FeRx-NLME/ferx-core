...
/// Which engine writes the disposition a [`StructuralSpec`] describes (#1257).
///
/// The analytic templates cover `ABSORPTION(INST | FO)`, `PERIPHERALS(0..2)`,
/// `TRANSITS` and `LAGTIME`, and nothing else: `ABSORPTION(ZO)`,
/// `ABSORPTION(WEIBULL)` and every `ELIMINATION` other than first-order have
/// no `pk NAME(...)` spelling at all. Those are reachable only as an
/// `ode_template NAME(...)` line plus one `[odes]` override, which is what
/// [`Ode`](Self::Ode) writes — the same disposition, generated by
/// [`crate::pk::ode_template`], with the central compartment's input and
/// elimination terms replaced.
///
/// The two directions are both edits: a candidate may move from an analytic
/// base to an ODE one and back, and the applier rewrites the template line
/// and the `[odes]` block accordingly rather than leaving a stale override
/// behind.
#[derive(Debug, Clone, PartialEq, Eq, Default)]
pub enum StructuralEngine {
    /// `pk NAME(role=VAR, …)` — the analytic closed form.
    #[default]
    Pk,
    /// `ode_template NAME(role=VAR, …)` plus the `[odes]` override the
    /// variant needs. `Ode { input: Template, elimination: FirstOrder }` is
    /// the generated disposition unchanged — a numerically-integrated twin of
    /// the analytic template, which is a legitimate thing to ask for and
    /// writes no override line at all.
    Ode {
        input: InputForm,
...
/// The `[structural_model]` template line, plus the defaults for any
/// `[individual_parameters]` value the new template needs and the model does
/// not yet have.
///
...
///
/// `#[non_exhaustive]`: build one with [`StructuralSpec::new`] and the
/// [`ode`](StructuralSpec::ode) builder, so the next field this struct grows
/// (as `engine` did on #1257) does not break a caller's struct literal. The
/// fields stay public to read.
#[derive(Debug, Clone, PartialEq)]
#[non_exhaustive]
pub struct StructuralSpec {
    /// A template name, e.g. `two_cpt_oral`. Validated against the same name
    /// table the parser uses, so a typo fails at edit time.
    pub template: String,
    /// `role = variable` bindings in the order they should be written, e.g.
    /// `[("cl", "CL"), ("v1", "V1"), ("q", "Q"), ("v2", "V2"), ("ka", "KA")]`.
    ///
    /// The `f` and `lagtime` roles are written differently by the two
    /// engines and stated the same way here: a `pk` line carries them as
    /// roles, while `ode_template` takes only the disposition roles and the
    /// engine reads bioavailability and lag time off an
    /// `[individual_parameters]` name instead — so the ODE applier emits the
    /// reserved-name alias (`lagtime = TLAG`) the ODE twin builder uses
    /// (#735), and omits it when the bound variable already carries a
    /// reserved spelling (`F`, `ALAG`, `LAGTIME`).
    ...
    /// exists is ignored.
    ///
    /// A parameter an [`Ode`](StructuralEngine::Ode) variant reads but no
    /// binding names — the Michaelis constant, the Weibull shape — belongs
    /// here too: it is declared, and the `[odes]` override is what references
    /// it.
    pub new_parameters: Vec<NewParameter>,
    /// Which engine writes the disposition. Defaults to
    /// [`StructuralEngine::Pk`].
    pub engine: StructuralEngine,
}

impl StructuralSpec {
    /// An analytic `pk NAME(role=VAR, …)` swap.
    pub fn new(
        template: impl Into<String>,
        bindings: Vec<(String, String)>,
        new_parameters: Vec<NewParameter>,
    ) -> Self {
        StructuralSpec {
            template: template.into(),
            bindings,
            new_parameters,
            engine: StructuralEngine::Pk,
        }
    }

    /// Write the disposition as `ode_template NAME(…)` with the given central
    /// input and elimination terms instead.
    pub fn ode(mut self, input: InputForm, elimination: EliminationForm) -> Self {
        self.engine = StructuralEngine::Ode { input, elimination };
        self
    }
}
