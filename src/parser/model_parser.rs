...
fn apply_ode_template(extracted: &mut ExtractedBlocks) -> Result<(), String> {
    ...
    // User-declared `d/dt(X)` lines replace the generated equation for X. Reuse
    // `diffeq_state` (the parser's own definition of a state equation) so the
    // override keys off exactly what `build_ode_spec` treats as a `d/dt`.
