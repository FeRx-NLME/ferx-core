...
/// B9 (#1496) — routing. A `CENS` cell that is not a whole number, on an
/// observation row `ferx check --data` will read, is a reader `Err`, and
/// `validate_model_file` relays a reader `Err` that states no code of its own as
/// **`E_DATA`** (`api/validation.rs`, `covariate_read_diagnostic`'s fallback arm).
/// The report is therefore invalid, which is the CLI's exit code 1
/// (`crates/ferx-cli/src/main.rs`: `if report.valid { 0 } else { 1 }`).
///
/// The oracle for the reject itself is the unit test on the reader
/// (`a_non_whole_cens_cell_on_a_scored_observation_row_is_an_error`); this pins
/// that the reject reaches the `ferx check` surface under a named code, and that
/// the reader's own sentence survives the relay rather than being replaced by a
/// generic one. The clean twin — the same file with `CENS=1` — is here so the
/// assertion cannot pass on a model or dataset that was invalid for some other
/// reason.
#[test]
fn a_non_whole_cens_cell_is_an_e_data_error_in_the_check_report() {
    const CENS_MODEL: &str = "\
[parameters]
  theta TVCL(0.2, 0.001, 10.0)
  theta TVV(10.0, 0.1, 500.0)
  omega ETA_CL ~ 0.09
...
";
    ...
    let model = temp_model("cens_not_whole", CENS_MODEL);
    let mp = model.to_str().unwrap().to_string();

    // The twin that must stay valid: `1` is in the domain, so nothing here is
    // rejected for any other reason.
    let ok_data = temp_data("cens_not_whole_ok", &csv("1"));
    let ok = validate_model_file(&mp, Some(ok_data.to_str().unwrap()));
    assert!(
        ok.valid,
        "an in-domain CENS column must leave the report valid: {:?}",
        ok.diagnostics
    );

    let bad_data = temp_data("cens_not_whole_bad", &csv("abc"));
    let bad = validate_model_file(&mp, Some(bad_data.to_str().unwrap()));
    assert!(
        !bad.valid,
        "a non-whole CENS cell is an error, so exit code 1: {:?}",
        bad.diagnostics
    );
    let d = bad
        .diagnostics
        .iter()
        .find(|d| d.code == "E_DATA")
        .unwrap_or_else(|| {
            panic!(
                "no E_DATA in the report; it carried {:?}",
                bad.diagnostics
                    .iter()
                    .map(|d| d.code.as_str())
                    .collect::<Vec<_>>()
    ...
    // The reader's own sentence, relayed rather than restated.
    assert!(d.message.contains("CENS=\"abc\""), "{}", d.message);
    assert!(d.message.contains("subject 1"), "{}", d.message);
    assert!(d.message.contains("Fix the cell"), "{}", d.message);

    let _ = std::fs::remove_file(&model);
    let _ = std::fs::remove_file(&ok_data);
    let _ = std::fs::remove_file(&bad_data);
}
