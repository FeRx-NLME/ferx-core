...
/// closer to 0 (monotonically). Output-modulator variance is a downstream
/// consequence — we assert it stays non-increasing and ends near 0, with an
/// absolute tolerance so the check is meaningful even when the unregularized fit
/// already produces little spread on this particular dataset.
#[test]
#[cfg_attr(
    not(feature = "slow-tests"),
    ignore = "slow: opt in with --features slow-tests"
...
fn l2_shrinks_weights_and_modulator_variation() {
    ...
    // with λ (observed here ~29.7 → ~2.1 → ~0.3 — a ~90× collapse). This is the
    // guaranteed mechanism by which L2 flattens the covariate→modulator map.
    assert!(
    ...
    // On this dataset the CL covariate signal is essentially null, so the CL
    // modulator variance is already ~0 unregularized (the fit reaches its large
    // weight norm without producing across-subject CL spread). Under strong L2
    // the network collapses toward a constant map, so the modulator stays
    // essentially flat — we assert only that, not strict monotonicity between two
    // numerically-zero baselines.
    assert!(
        v_big < 1e-3,
        "under strong L2 the CL modulator must stay essentially flat (var {v_big:.6})"
    );
}
