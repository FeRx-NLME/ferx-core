...
/// Fast guard: a typo in `adaptive_platelet_ladder.ferx` — which exercises new DSL
/// surface, notably the one-sided `target_window = [100, inf]` — fails here, by
/// name, rather than as a mismatch in the cross-engine check below. `parse_full_model_file` runs the full
