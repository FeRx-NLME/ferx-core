...
| **`[parameters]` rejects what it cannot parse** | a scale tag - or any other trailing text - on a `block_omega` / `block_sigma` / `block_kappa` declaration, and a line matching no declaration form at all - [#1377](https://github.com/FeRx-NLME/ferx-core/issues/1377). **Not a fit anchor, and the only row here that is not: the object under test is a parse outcome, so there is no number to compare.** What the two streams record is that the comparator also treats these shapes as errors rather than ignoring them, which is the whole question #1377 asks - ferx used to drop both in silence and report the file VALID. Each stream is a valid warfarin `ADVAN2` `MAXEVAL=0` evaluation differing from an accepted one by exactly one token, in the same vary-one-input-at-a-time discipline as the rest of this table, so neither needs a separate baseline file: arm A appends `FOO` to the `$OMEGA BLOCK` record, arm B puts `BANANA` on a line of its own. NM-TRAN answers both with `AN ERROR WAS FOUND IN THE CONTROL STATEMENTS ... THE CHARACTERS IN ERROR ARE: FOO` / `: BANANA`. **Provenance: the NM-TRAN outputs are quoted from the measurement recorded in [#1377 section 0](https://github.com/FeRx-NLME/ferx-core/issues/1377#issuecomment-5659914911) (`nmfe76`, `nonmemdocker:V0.1`); the session that wrote these streams had no NONMEM available and did not re-run them.** Measured there and deliberately **not** anchored here: NM-TRAN *accepts* `$OMEGA BLOCK(2) SD 0.3 0.02 0.2`, squaring the diagonal only and leaving the off-diagonal a covariance - a widening tracked as [#1387](https://github.com/FeRx-NLME/ferx-core/issues/1387), not part of this change | `parameters_reject_trailing_token.ctl`, `parameters_reject_stray_line.ctl` (2) | *(in `tests/check_command.rs` and `src/parser/model_parser_tests.rs`)* |
...
## Dose-compartment anchors (#375)

`dose_cmt_*.ctl` — 13 evaluation-only runs (`$ESTIMATION MAXEVAL=0`, `$OMEGA 0 FIX`,
...
|------|-------|--------------|
| `ss_cmt0`, `ss_cmt1` | 1 | `SS=1 II=12`: NONMEM prints **identical** `PRED` for `CMT=0` and `CMT=1`, so `CMT=0` is the *default dose compartment* and must equilibrate like any other |
| `advan2_bolus_cmt2` | 2 | oral model, bolus straight into central |
| `advan3_bolus_cmt2` | 3 | 2-cpt IV, bolus into the peripheral |
| `advan4_bolus_cmt3` | 4 | 2-cpt oral, bolus into the peripheral |
| `advan11_bolus_cmt3` | 11 | 3-cpt IV, bolus into peripheral-2 |
| `advan12_bolus_cmt2` | 12 | 3-cpt oral, bolus into central |
| `advan3_inf_cmt2` | 3 | `RATE=20` into an IV peripheral |
| `advan4_inf_cmt3` | 4 | `RATE=20` into an **oral** peripheral — prints the same `PRED` as `advan3_inf_cmt2`, confirming the oral propagator may reuse the IV forced response (the depot is empty throughout) |
| `advan12_inf_cmt3`, `advan12_inf_cmt4` | 12 | `RATE=20` into an oral 3-cpt peripheral-1 / peripheral-2 |
